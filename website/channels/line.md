...
`/help` lists the commands available in the current mode. Session commands include `/session`, `/sessions`, `/clear`, `/compact`, `/restart`, `/stop`, `/model` and `/models`. `/compact` and `/restart` first show **Yes / No** postback buttons. Confirmation expires after five minutes and is bound to the requesting user, chat and active session; it can be used once. `/restart` restarts the session process, not the gateway service or the app container. Chat history is preserved.
