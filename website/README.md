...
This edition documents the latest Agent Orchestration Engine code (source revision `4a81967`) as the normal product, including voice and multi-channel operation. The documentation PR is independent of runtime code: do not copy gateway implementation files into it merely to change the documentation baseline. Verify edits against the implementation revision documented here. API reference pages under `api/` replace the former root `API.md`; keep route documentation there. The command reference at `reference/cli.md` replaces root `CLI.md` and remains generated from the CLI source. Never publish ignored private notes as site content.
...
CI builds and checks only this website. The deployment workflow publishes checked builds to GitHub Pages on main changes or a manual dispatch on main. PR builds cannot deploy. Initial publication uses the `gh-pages` branch while this PR remains open; before the first main deployment, a repository administrator must select **Settings → Pages → Build and deployment → Source → GitHub Actions**. The workflow deliberately does not change repository settings: its `GITHUB_TOKEN` has Pages deployment permissions, not the Administration permission required to change the Pages source. Keep the existing branch publication until this one-time switch. To prepare assets for a host subpath, run `DOCS_BASE=/claude-gateway/ npm run check`; the resulting `.vitepress/dist` targets the project Pages URL. The default base `/` supports a standalone domain. Do not place generated output in the gateway's runtime `dist`.
