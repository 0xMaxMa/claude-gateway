...
Overview shows hourly token activity with hover/focus details and a stacked bar per agent, separating agent and worker tokens. Both use recorded usage for today in `gateway.timezone`, including cache, rather than billing cost. Needs your attention appears only when tasks await input or reconciliation; inspect details in the dashboard and respond in the original agent conversation. Theme controls and full-height detail drawers are shared across dashboard pages and the standalone token report.
