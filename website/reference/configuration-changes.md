...
| --- | --- |
| `gateway.jev`, `agents[].jev` | New evaluations capture current provider/model/limits. Global disablement and access revocation are checked before dispatch and return; key files rotate without restart. Existing process tool inventories may require a new process to expose a newly enabled tool |
