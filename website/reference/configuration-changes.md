...
| --- | --- |
| `gateway.jev`, `agents[].jev` | New evaluations capture current provider/model/limits. Global disablement and access revocation are checked before dispatch and return; key files rotate without restart. Browser binding changes/revocation are checked live; controller credentials rotate on new connections. Experience settings apply to new browser requests; disabling learning prevents later promotion. Installed Logic code updates need restart. Existing process tool inventories may require a new process to expose a newly enabled tool |
