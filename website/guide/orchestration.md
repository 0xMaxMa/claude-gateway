...
Each card has independent date controls, saved separately for each chart in this browser and dashboard path, including across reloads and new tabs: **24h / 7d / 30d / 90d**, defaulting to **24h**. **24h starts at 00:00 in `gateway.timezone`**; longer ranges include today. While Overview is visible, charts refresh every 10 seconds and share requests for identical ranges. Changing one card does not change the other cards or the session list. All four charts use shared tooltips with muted labels and bold values colored to match their series. Refresh preserves keyboard focus on the same data point and updates open tooltips; if a focused point disappears, focus returns to the selected range button.
...
- **Tokens by agent:** vertical bars ranked by recorded token volume. Each bar is 32 px wide; card width determines the top 2–8 agents shown. The caption states how many are included. Bars use the same identity colors as the Agents badges. Hover over the bar or directly above it within its 32 px width, or focus the column, for the full agent name and role split.
- **Tokens by model:** total Agent + Worker volume grouped by the model recorded on each turn. The top five models are named; the remainder are **Other models**. Missing identifiers appear as **Unspecified model**. Hover or focus a donut segment for an immediate tooltip showing the model, tokens and share. The donut and model list share equal columns on desktop.
- **Context reuse:** three lines for cache read, cache write and fresh input, using the same hourly points for today and daily points for longer ranges as Token activity. Hover or focus a point for each component’s percentage of measured input and token count; percentages use that point’s cache read + cache write + fresh input, not the entire selected range. Zero input displays **—**, not an inferred percentage. The headline always shows **today’s** measured input reuse, independently of the selected chart range, compared with **yesterday’s full day** in percentage points. For example, 80% yesterday and 86.3% today is up 6.3 percentage points. Today is still in progress; this is not a matched-time-of-day comparison. Reuse is `cache read / (fresh input + cache write + cache read)`. **Reused** means cache read; **New · saved** means cache write; **New · not cached** means fresh input. Output and turns missing a breakdown field are excluded; the caption counts missing measurements for today. With no measured input, the percentage is **—** and no comparison is inferred. This measures input reuse, **not a percentage of money saved**.
