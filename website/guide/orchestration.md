...
Each card has independent date controls, retained for the browser tab: **24h / 7d / 30d / 90d**, except Context reuse, which offers **7d / 30d / 90d** and defaults to **7d**. **24h starts at 00:00 in `gateway.timezone`**; longer ranges include today. While Overview is visible, charts refresh every 10 seconds and share requests for identical ranges. Changing one card does not change the other cards or the session list.
...
- **Context reuse:** one stacked bar per calendar day, with three segments (cache read, cache write, fresh input). Longer ranges scroll horizontally, initially showing the latest days; hover directly above a bar or focus it for daily totals. The period-wide reuse percentage is `cache read / (fresh input + cache write + cache read)`. **Reused input** means cache read; **New · saved for reuse** means cache write; **New · not cached** means fresh input. Output is excluded. Turns missing a breakdown field are excluded from the ratio and counted in the caption. This measures input reuse, **not a percentage of money saved**.
