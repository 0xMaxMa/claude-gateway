...
Session token reports refresh visible data every five seconds while the tab is visible, preserving filters, expanded turn details and theme. Provider usage is broken down by role into fresh input, cache write, cache read and output across the selected scope. Context bootstrap is a separate estimate: the `cl100k_base` reference tokenizer measures current generated workspace sections, source files and host Agent gateway tool schemas in the reader thread, cached for 30 seconds. It is not the selected model's exact tokenizer or historical usage attribution. Source files and sections must not be added to the parent CLAUDE.md total. Conversation history, runtime overlays, native CLI instructions/tools and provider framing are excluded; container-only workspaces and unreadable/oversized files remain unavailable. No model requests are made for this measurement.

Dashboard tables expand inline when the row is clicked (Enter/Space also work) and retain expansion across refresh. Session IDs are displayed in full. Missing values render as —. Tools shown as **Used** count distinct observed names, not calls. The legacy JSON `loadedTools` field is the available inventory reported by CLI `system.init`; it does not prove that those schemas were included in a model request. The UI shows **Loaded: —** because that measurement is not reported, and lists the available inventory separately in expanded details. Agent/Worker token columns are role splits of Totals, not extra tokens to add. Failed agent turns include a recorded response failure code where available; a failed report turn does not imply a failed worker.
