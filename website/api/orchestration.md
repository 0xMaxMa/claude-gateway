...
The Context window card uses the latest observed request in the newest agent turn, including output, rather than the pre-compaction peak. This is the latest recorded request measurement, not an exact live tokenizer reading. Auto-refresh updates the session header status as well as totals and turn rows.
