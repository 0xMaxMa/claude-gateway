...
`GET /api/v1/agents/:agentId/voice-settings/catalog?provider=upstream:gemini` lists connected Gemini models and prebuilt voices. Gemini TTS uses generateContent; Gemini browser STT uses realtime partial transcripts with `gemini-3.5-transcribe-live`; other Gemini STT models transcribe recorded segments after pause/mute. Uploaded voice messages use a recorded-file model configured in `voice.notes`. Direct Gemini uses `GEMINI_API_KEY`; upstream Gemini reuses the pod provider credentials.
