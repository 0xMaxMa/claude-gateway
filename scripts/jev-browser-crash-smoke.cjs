...
function adapter(options,root,evaluate){
 ...
 const binding={version:1,id:'fixture',name:'Crash fixture',principalId:'fixture-user',conversationId:'fixture-chat',run:c=>executeBrowserModule(options.runnerModule,connector,c),inspect:(result,signal,authorized)=>inspectBrowser(connector,result,signal,authorized)};
...
async function runGatewayBrowserCrashFixture(options){
 ...
 try{
  ...
  child.send({type:'start',options:{endpoint:options.endpoint,credentialFile:options.credentialFile,scope:options.scope,runnerModule:options.runnerModule,goal:options.goal},root});
