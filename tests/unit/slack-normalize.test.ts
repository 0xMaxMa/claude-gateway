...
describe('inbound image download → meta.image_path', () => {
  ...
  test('orchestration keeps multiple general files, names and bytes instead of silently dropping PDFs',async()=>{
    const runner=fakeRunner(),config=runner.getAgentConfig();config.orchestration={enabled:true,channels:['slack']};runner.getAgentConfig=()=>config;
    handler=createSlackWebhookHandler(new Map([[AGENT,runner]]),'/tmp');downloadResponse=()=>new Response('%PDF-fixture');
    await post(imageEvent([{id:'p1',name:'one.pdf',mimetype:'application/pdf',url_private:PRIVATE_URL},{id:'p2',name:'two.csv',mimetype:'text/csv',url_private:PRIVATE_URL}]));
    const files=JSON.parse(forwarded[0].meta.attachments_json);written.push(...files.map((f:any)=>f.path));
    expect(files).toHaveLength(2);expect(files.map((f:any)=>f.name)).toEqual(['one.pdf','two.csv']);
    expect(files[0].path).toMatch(/\.pdf$/);expect(fs.readFileSync(files[0].path,'utf8')).toBe('%PDF-fixture');
  });
