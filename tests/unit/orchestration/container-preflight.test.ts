...
test('Docker preflight rejects namespaces, privilege and alternate host mounts',async()=>{
 ...
 try {
  ...
  for(const patch of [{Privileged:true},{NetworkMode:'host'},{NetworkMode:'container:other'},{PidMode:'host'},{IpcMode:'host'},{CapAdd:['SYS_ADMIN']},{CapDrop:[]},{SecurityOpt:[]},{SecurityOpt:['no-new-privileges=false']},{SecurityOpt:['no-new-privileges:true','no-new-privileges=0']},{SecurityOpt:['no-new-privileges-invalid']}]){mockInspect=healthy();Object.assign(mockInspect.HostConfig,patch);await expect(validateContainer(agent)).rejects.toMatchObject({code:'CONTAINER_ISOLATION_REQUIRED'});}
...
test.each([
 ...
 [['no-new-privileges:false','no-new-privileges=TRUE'],true],
 [['no-new-privileges=true','no-new-privileges=false'],false],
 [['no-new-privileges=1'],true],
 [['no-new-privileges=0'],false],
 [['no-new-privileges=true','no-new-privileges=invalid'],false],
] as Array<[string[],boolean]>)('evaluates effective no-new-privileges option %j', async (options, expected) => {
 const {hasNoNewPrivileges}=await import('../../../src/orchestration/container');
 expect(hasNoNewPrivileges(options)).toBe(expected);
});
