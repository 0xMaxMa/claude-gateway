...
test.each(['image','file'] as const)('linked WhatsApp sends %s bytes from staged media on the original account',async kind=>{
 const {AgentRunner}=await import('../../../src/agent/runner');
 const root=mkdtempSync(join(tmpdir(),'linked-media-'));
 try{
  mkdirSync(join(root,'a','media'),{recursive:true});
  const path=join(root,'a','media',kind==='image'?'image.png':'report.pdf');writeFileSync(path,'fixture');
  const sendWhatsAppMessage=jest.fn(async()=>{});
  const runner=Object.assign(Object.create(AgentRunner.prototype),{agentConfig:{...config,workspace:join(root,'a','workspace')},sendWhatsAppMessage});
  const file={path,name:kind==='image'?'image.png':'report.pdf',kind,caption:'Report'};
  await expect(runner.sendLinkedOrchestrationChannel({channel:'whatsapp',chat_id:'chat',thread_key:'whatsapp-account:original'},'','id',file)).resolves.toEqual({state:'delivered'});
  expect(sendWhatsAppMessage).toHaveBeenCalledWith('chat','Report',path,'original',{asDocument:kind==='file'});
  await expect(runner.sendLinkedOrchestrationChannel({channel:'wechat',chat_id:'chat'},'','id',file)).resolves.toEqual({state:'failed',code:'CHANNEL_ATTACHMENTS_UNSUPPORTED'});
 }finally{rmSync(root,{recursive:true,force:true});}
});
