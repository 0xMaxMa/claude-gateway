...
test('recovered reservations are retained until cleanup is confirmed and released exactly once', async () => {
 const f=fixture(), release=jest.fn(), reservations=new Map([[f.task.taskId, release]]);
 const scheduler=new WorkerScheduler(f.tasks,{cleanup:jest.fn().mockResolvedValueOnce(false).mockResolvedValue(true),start:jest.fn()},undefined,reservations);
 try {
  f.controls.cancel('s','owner',f.task.taskId);await scheduler.tick();
  expect(release).not.toHaveBeenCalled();expect(reservations.size).toBe(1);
  f.controls.cancel('s','owner',f.task.taskId);await scheduler.tick();await scheduler.tick();
  expect(f.store.task(f.task.taskId)!.state).toBe('cancelled');
  expect(release).toHaveBeenCalledTimes(1);expect(reservations.size).toBe(0);
 }finally{await scheduler.close();f.store.close();}
});
