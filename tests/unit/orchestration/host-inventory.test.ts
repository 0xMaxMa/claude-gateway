...
test.each([true, false])('agent intake tool requires the matching feature profile: %s', async semanticIntake => {
  const process = new EventEmitter() as SessionProcess;
  Object.assign(process, {runtimeProfile:{role:'agent',semanticIntake},start:async()=>{},stop:jest.fn(async()=>{}),
    sendMessage:()=>{
  ...
  const turn=startProcessTurn(process,'task',1000);
  if(semanticIntake)await expect(turn.result).resolves.toMatchObject({text:'done'});
  else await expect(turn.result).rejects.toMatchObject({code:'PROFILE_INVENTORY_MISMATCH'});
});
