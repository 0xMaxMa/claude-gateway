...
test('reopening an uncertain worker reuses its capacity reservation until confirmed cleanup',()=>{
 const capacity=new ProcessCapacity(2,1);
 const original=capacity.acquireWorker('a','task')!;
 const recovered=capacity.acquireWorker('a','task',false)!;
 expect(capacity.count).toBe(1);expect(recovered).toBe(original);
 expect(capacity.acquireWorker('a','other')).toBeUndefined();
 recovered();expect(capacity.count).toBe(0);
 const next=capacity.acquireWorker('a','task')!;original();
 expect(capacity.count).toBe(1);expect(capacity.acquireWorker('a','task',false)).toBe(next);
 next();expect(capacity.count).toBe(0);
});
