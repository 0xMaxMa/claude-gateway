...
test('a later final answer sorts after its acknowledgement in canonical history', async () => {
 const root=mkdtempSync(join(tmpdir(),'ack-history-'));
 const store=new OrchestrationStore(':memory:','a'),sessions=new SessionStore(root),history=HistoryDB.forAgent(root,'a');
 const now=jest.spyOn(Date,'now');
 try {
  now.mockReturnValue(1000);
  const input=store.acceptInput({scope:{agentId:'a',agentSessionId:'s',source:'api',accountId:'key',chatId:'c',threadKey:'',principalId:'p'},text:'Please review'});
  const decisions=new DecisionService(store),decision=decisions.begin(input.conversationId,'p',[input.inputId]);
  now.mockReturnValue(2000);const ack=decisions.acknowledge(decision,'I am reviewing it.');
  now.mockReturnValue(3000);decisions.finish(decision,'Here is the answer.');
  const writer=new OrchestrationHistoryWriter(store,sessions,history);
  // Even if outbox recovery writes the final response first, display timestamps
  // must reflect acknowledgement then answer, not the decision start time.
  await writer.write(`input:${input.inputId}`);
  await writer.write(`response:${decision.responseId}`);
  await writer.write(`response:${ack}`);
  const chronological=(await sessions.loadSession('a','s')).sort((a,b)=>a.ts-b.ts);
  expect(chronological.map(m=>m.content)).toEqual(['Please review','I am reviewing it.','Here is the answer.']);
  expect(chronological.map(m=>m.ts)).toEqual([1000,2000,3000]);
  expect(history.getMessages('api-c',{sessionId:'s'}).messages.map(m=>m.content)).toEqual(chronological.map(m=>m.content).reverse());
 }finally{now.mockRestore();store.close();(history as any).db.close();HistoryDB.evict(root,'a');rmSync(root,{recursive:true,force:true});}
});
