...
test('native skill discovery does not silently discard entries after 1000',()=>{
 const entries=Array.from({length:1100},(_,i)=>({name:`skill-${i}`,description:'Installed skill'}));
 expect(parseCliSkills(entries)).toHaveLength(1100);
});
