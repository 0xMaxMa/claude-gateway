...
test('enabled Jev defaults to scoped managed discovery and never direct-worker fallback',()=>{
 const gateway={gateway:{customConnectors:{},jev:{enabled:true,features:{browserTasks:{enabled:true}},browser:{runnerModule:'runner',bindings:[]}}}} as unknown as GatewayConfig;
