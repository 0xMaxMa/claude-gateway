...
test('an early timer rechecks the quiet deadline instead of losing its observation',async()=>{
 jest.useFakeTimers();
 const clock=jest.spyOn(Date,'now');
 try {
  clock.mockReturnValue(1000);
  const observer=jest.fn(),{turn}=fixture(observer);
  clock.mockReturnValue(1099);
  await jest.advanceTimersByTimeAsync(100);
  expect(observer).not.toHaveBeenCalled();
  clock.mockReturnValue(1100);
  await jest.advanceTimersByTimeAsync(1);
  expect(observer).toHaveBeenCalledWith(expect.objectContaining({quiet:true,activeTools:['Bash']}));
  await turn.stop();await turn.result;
 } finally {clock.mockRestore();jest.useRealTimers();}
});
