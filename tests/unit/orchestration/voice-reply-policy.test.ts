...
test.each([['voice_note','text',false],['text','voice_note',true]] as const)('updated task report follows %s → %s', (first, second, expected) => {
 const s=new OrchestrationStore(':memory:','a'), decisions=new DecisionService(s), tasks=new TaskService(s);
 const scope={agentId:'a',agentSessionId:'s',source:'telegram' as const,accountId:'bot',principalId:'p',chatId:'c',threadKey:''};
 try {
  const i=s.acceptInput({scope,text:'Do work',modality:first}),d=decisions.begin(i.conversationId,'p',[i.inputId]);
  const task=tasks.spawn({...i,...d,principalId:'p',execute:true,writeMemory:false,actionId:'spawn'}, {title:'Work',instructions:'Original brief',targetProfile:'default-worker'});
  decisions.finish(d,'Working');
  const update=s.acceptInput({scope,text:'Include the appendix',modality:second}),ud=decisions.begin(update.conversationId,'p',[update.inputId]);
  tasks.update({...update,...ud,principalId:'p',execute:true,writeMemory:false,actionId:'update'},task.taskId,1,'Original brief plus appendix','when_ready');
  decisions.finish(ud,'Including the appendix');
  const attempt=tasks.claim(task.taskId)!;
  tasks.finish(attempt.attemptId,attempt.generation,{type:'completed',result:{summary:'Done',artifactIds:[]}});
  const report=s.acceptInput({scope,text:'Report',storeUserMessage:false}),rd=decisions.begin(report.conversationId,'p',[report.inputId]);
  expect(responseHasVoiceOrigin(s,rd.responseId!)).toBe(expected);
 } finally {s.close();}
});
