...
test('Gemini STT and TTS failures identify the operation', async () => {
  process.env.GEMINI_API_KEY = 'test';
  const request = jest.fn(async () => new Response('{}', { status: 503 })) as unknown as typeof fetch;
  const { transcribeGemini } = await import('../../../src/voice/providers/gemini');
  await expect(transcribeGemini(Buffer.alloc(64), { provider: 'gemini', model: 'gemini-2.5-flash', signal: new AbortController().signal }, request)).rejects.toThrow('STT_PROVIDER_ERROR_HTTP_503');
  const audio = new GeminiTts('gemini', 'gemini-3.1-flash-tts-preview', request).synthesize({ text: (async function* () { yield 'Hello'; })(), voiceId: 'Kore', outputFormat: PCM16, signal: new AbortController().signal });
  await expect(audio.next()).rejects.toThrow('TTS_PROVIDER_ERROR_HTTP_503');
});

test('dedicated Gemini transcription reads audioTranscription, without prompting the recognizer', async () => {
 process.env.GEMINI_API_KEY='test';
 const request=jest.fn<ReturnType<typeof fetch>,Parameters<typeof fetch>>(async()=>new Response(JSON.stringify({candidates:[{finishReason:'STOP',content:{parts:[{audioTranscription:{text:'สวัสดีค่ะ'}}]}}]})));
 const {transcribeGemini}=await import('../../../src/voice/providers/gemini');
 expect(await transcribeGemini(Buffer.from('OggS1234'),{provider:'gemini',model:'gemini-3.5-transcribe',language:'th',signal:new AbortController().signal},request)).toBe('สวัสดีค่ะ');
 const body=JSON.parse(request.mock.calls[0][1]!.body as string);
 expect(body.contents[0].parts).toHaveLength(1);expect(body.contents[0].parts[0].text).toBeUndefined();expect(body.generationConfig.audioTranscriptionConfig.languageCodes).toEqual(['th']);
});
