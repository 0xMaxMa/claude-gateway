...
test('OpenRouter catalog preserves vendor/model IDs and excludes cloning-only choices', async () => {
  const catalog = await import('../../../src/voice/providers/voice-catalog');
  const models = await import('../../../src/voice/providers/openrouter');
  const choices = jest.spyOn(catalog, 'voiceChoices').mockResolvedValue([{ id: 'Kore', name: 'Kore', supported_model_ids: ['google/tts'] }]);
  const listed = jest.spyOn(models, 'openRouterVoiceModels').mockResolvedValue([
    { id: 'google/tts', architecture: { output_modalities: ['speech'] }, supported_voices: ['Kore'] },
    { id: 'fish/cloning', architecture: { output_modalities: ['speech'] } },
    { id: 'openai/whisper-1', architecture: { output_modalities: ['transcription'] } },
  ]);
  const app = express();
  app.use(voiceSettingsRouter(new Map([['a', { id: 'a' } as AgentConfig]]), new Map(), [{ id: 'reader', key: 'read', agents: ['a'] }]));
  try {
    const response = await request(app).get('/v1/agents/a/voice-settings/catalog').query({ provider: 'upstream:openrouter' }).set('Authorization', 'Bearer read');
    expect(response.status).toBe(200);
    expect(response.body.models.map((m: { model_id: string }) => m.model_id)).toEqual(['openrouter/google/tts', 'openrouter/openai/whisper-1']);
    expect(response.body.models[1]).toMatchObject({ realtime: false, voice_messages: true });
    expect(response.body.voices[0].supported_model_ids).toEqual(['google/tts']);
    expect(response.body.warnings.join(' ')).toContain('cloning');
  } finally { choices.mockRestore(); listed.mockRestore(); }
});
...
test('voice settings require scoped writes and persist overrides without clobbering other config', async () => {
  ...
  try {
    ...
    expect(response.body.voice).toMatchObject({
      enabled: true,
      language: 'ja',
      tts: { provider: 'upstream:elevenlabs', voiceId: 'inherited' },
    });
...
test.each(['upstream','upstream:elevenlabs'])('catalog accepts %s and returns the canonical provider namespace',async provider=>{
  const catalog=await import('../../../src/voice/providers/voice-catalog');
  const choices=jest.spyOn(catalog,'voiceChoices').mockResolvedValue([{id:'chosen',name:'Chosen'}]);
  const connection=await import('../../../src/voice/providers/upstream');
  const upstream=jest.spyOn(connection,'upstreamVoiceConnection').mockReturnValue({base:new URL('https://relay.test/v1/voice/elevenlabs/'),key:'fixture'});
  const fetcher=jest.spyOn(global,'fetch').mockResolvedValue(new Response(JSON.stringify([{model_id:'eleven_v3',name:'Eleven v3',can_do_text_to_speech:true}])));
  const app=express();app.use(voiceSettingsRouter(new Map([['a',{id:'a'} as AgentConfig]]),new Map(),[{id:'reader',key:'read',agents:['a']}]));
  try{
    const response=await request(app).get('/v1/agents/a/voice-settings/catalog').query({provider}).set('Authorization','Bearer read');
    expect(response.status).toBe(200);
    expect(response.body.models).toEqual(expect.arrayContaining([expect.objectContaining({provider:'elevenlabs',model_id:'elevenlabs/eleven_v3',source:'byok'})]));
    expect(choices).toHaveBeenCalledWith({provider:'upstream:elevenlabs',voiceId:''});
  }finally{choices.mockRestore();upstream.mockRestore();fetcher.mockRestore();}
});
