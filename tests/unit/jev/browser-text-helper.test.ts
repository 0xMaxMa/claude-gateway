...
test('helper config and credential isolation',()=>{
 validateBrowserIntegration({runnerModule:'runner',bindings:[],textHelper:config});
 expect(()=>validateBrowserIntegration({runnerModule:'runner',bindings:[],textHelper:{...config,apiKeyEnv:'ANTHROPIC_API_KEY'}})).toThrow();
 expect(sanitizeJevChildEnv({TEST_BROWSER_TEXT_SECRET:'test-key'},{browser:{runnerModule:'runner',bindings:[],textHelper:config}})).toEqual({});
});
