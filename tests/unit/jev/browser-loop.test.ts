...
const config=():BrowserIntegrationConfig=>({bindings:[{id:'tab',name:'Private browser',agentId:'alpha',principalId:'owner',conversationId:'chat',endpoint:'https://browser.example/mcp',apiKeyEnv:'PRIVATE_BROWSER_TOKEN',scope:{device_id:'device',grant_id:'grant',tab_id:'tab'}}]});
test('supports installed package exports or absolute module paths, not remote code',()=>{
 ...
 for(const adapterModule of ['https://bad.example/code.js','../untrusted.js','a/b','@example/pkg/subpath'])expect(()=>validateBrowserIntegration({...config(),adapterModule} as any)).toThrow();
 expect(()=>validateBrowserIntegration({...config(),adapterModule:'/opt/runner/index.mjs'} as any)).toThrow();
});
