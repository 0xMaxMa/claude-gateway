...
test('web Stop returns numbered tasks; numeric selection stops one real scheduled handle without another user inference',async()=>{
 ...
 const app=express();app.use(express.json());app.use('/api',createApiRouter(new Map([['a',runner]]),new Map([['a',agent]]),[{id:'owner',key:'fixture',agents:['a'],allow_tools:true},{id:'intruder',key:'intruder',agents:['a'],allow_tools:true}]));
 ...
 try{
  ...
  const path = `/api/v1/agents/a/sessions/${sid}/tasks`;
  const listed = await request(app).get(path).set('Authorization','Bearer fixture');
  expect(listed.status).toBe(200);expect(listed.body.total).toBe(2);
  expect((await request(app).get(path+'?page_size=0').set('Authorization','Bearer fixture')).status).toBe(400);
  expect((await request(app).get(path).set('Authorization','Bearer intruder')).status).toBe(403);
  expect((await request(app).get(path+'/'+listed.body.tasks[0].taskId).set('Authorization','Bearer intruder')).status).toBe(403);
  expect((await request(app).post(path+'/'+listed.body.tasks[0].taskId+'/cancel').set('Authorization','Bearer intruder')).status).toBe(403);
  const shown = await request(app).get(path+'/'+listed.body.tasks[0].taskId).set('Authorization','Bearer fixture');
  expect(shown.status).toBe(200);expect(shown.body.task.skill).toBeUndefined();
  expect(shown.body.task.state).toBe('running');
  ...
  const flags=['--agent','a','--session',sid,'--url',`http://127.0.0.1:${(server.address() as AddressInfo).port}`,'--key','fixture','--json'];
  const output=jest.spyOn(process.stdout,'write').mockImplementation(()=>true);
  try {
    expect(await runCli(['tasks','list',...flags])).toBe(0);
    expect(JSON.parse(String(output.mock.calls.at(-1)![0])).total).toBe(1);
    expect(await runCli(['tasks','list','--all',...flags])).toBe(0);
    expect(JSON.parse(String(output.mock.calls.at(-1)![0])).total).toBe(2);
    expect(await runCli(['tasks','show',other,...flags])).toBe(0);
    expect(JSON.parse(String(output.mock.calls.at(-1)![0])).task.taskId).toBe(other);
    expect(await runCli(['tasks','cancel',other,...flags])).toBe(0);
    await until(()=>runtime.store.task(other)!.state==='cancelled');
    expect(await runCli(['tasks','watch',other,...flags])).toBe(0);
    expect(JSON.parse(String(output.mock.calls.at(-1)![0])).task.state).toBe('cancelled');
    expect(runtime.store.task(other)!.cancellation?.requestedBy).toBe('user');
    expect(userTurns).toBe(1);
    const legacyAgent = {...agent, orchestration:{enabled:false}};
    const legacyRunner = new AgentRunner(legacyAgent, {...gateway, gateway:{...gateway.gateway, orchestration:false}});
    const legacyApp=express();legacyApp.use(express.json());legacyApp.use('/api',createApiRouter(new Map([['a',legacyRunner]]),new Map([['a',legacyAgent]]),[{id:'owner',key:'fixture',agents:['a']}]));
    expect((await request(legacyApp).get(path).set('Authorization','Bearer fixture')).status).toBe(409);
    expect((await request(legacyApp).get(path+'/'+other).set('Authorization','Bearer fixture')).status).toBe(409);
    expect((await request(legacyApp).post(path+'/'+other+'/cancel').set('Authorization','Bearer fixture')).status).toBe(409);
  }finally{output.mockRestore();}

 }finally{if(server)await new Promise<void>(resolve=>server!.close(()=>resolve()));await runtime.close();(history as any).db.close();HistoryDB.evict(join(root,'agents'),'a');rmSync(root,{recursive:true,force:true});}
});
