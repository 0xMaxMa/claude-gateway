...
test.each([false, true])('API worker completion persists result, tools and optional live voice summary (voice=%s)', async voice => {
  ...
  const fullResult = 'verified worker result\n' + 'หลักฐานและรายละเอียด '.repeat(160) + '\nEND: skill and shared knowledge findings.';
    ...
    createAgentSession: async (_id, profile) => {
      ...
      process.sendMessage = prompt => {
        ...
        if (prompt.includes('Report the persisted')) {
          ...
          // this turn is speech-enabled — one invariant union schema, never a per-turn toggle.
  ...
  try {
    ...
    complete!({ type: 'completed', result: { summary: fullResult, artifactIds: [] } });
...
test('a failed automatic report retries only the persisted result, without running the worker again', async () => {
 const root=mkdtempSync(join(tmpdir(),'report-retry-runtime-'));
 const sessions=new SessionStore(root),history=HistoryDB.forAgent(root,'a');
 const agent={id:'a',workspace:join(root,'a/workspace'),description:'',env:'',claude:{model:'fixture',extraFlags:[]}} as AgentConfig;
 const gateway={gateway:{orchestration:true,headless:true,logDir:join(root,'logs'),timezone:'UTC'},agents:[agent]} as GatewayConfig;
 let runtime:AgentOrchestrationRuntime,reports=0,workers=0;
 runtime=await AgentOrchestrationRuntime.open(agent,gateway,root,sessions,history,{
  createAgentSession:async()=>{
   const process=new EventEmitter() as SessionProcess;
   process.start=async()=>{};process.stop=async()=>{};
   process.sendMessage=prompt=>{
    if(prompt.includes('Report the persisted')){
     ...
     process.emit('output',JSON.stringify(reports===1?{type:'result',is_error:true,result:'API Error: 503'}:{type:'result',result:'Reported successfully'}));
    }else{
     const d=runtime.store.get("SELECT * FROM conversation_decisions WHERE state='running'")!;
     runtime.tasks.spawn({conversationId:String(d.conversation_id),principalId:'p',inputId:JSON.parse(String(d.input_ids_json))[0],decisionId:String(d.id),epoch:Number(d.epoch),actionId:'spawn',execute:true,writeMemory:false},{title:'Fixture',instructions:'Work',targetProfile:'media-worker'});
     process.emit('output',JSON.stringify({type:'result',result:'Queued'}));
    }
   };return process;
  },releaseAgentSession:async()=>{},
 },{start:async()=>{workers++;return {accepted:Promise.resolve(),result:Promise.resolve({type:'completed',result:{summary:'Original complete result',artifactIds:[]}}),stop:async()=>{}};}});
 try{
  await runtime.send({scope:{agentId:'a',agentSessionId:'s',source:'api',accountId:'key',chatId:'c',threadKey:'',principalId:'p'},text:'Work'},{execute:true,writeMemory:false},{timeoutMs:1000});
  await until(()=>reports===1&&!runtime.isBusy('s'));
  await new Promise(r=>setTimeout(r,250));expect(reports).toBe(1);
  runtime.store.run("UPDATE conversation_decisions SET ended_at=ended_at-5000 WHERE state='failed'");
  await until(()=>reports===2&&!runtime.isBusy('s'));
  expect(workers).toBe(1);
  expect(runtime.store.get("SELECT COUNT(*) n FROM notifications WHERE status='pending'")!.n).toBe(0);
  expect(JSON.stringify(await sessions.loadSession('a','s'))).toContain('Reported successfully');
 }finally{await runtime.close();(history as any).db.close();HistoryDB.evict(root,'a');rmSync(root,{recursive:true,force:true});}
});
