...
**Voice through the configured provider.** Set `stt.provider`, `tts.provider` and `notes.provider` to `upstream:elevenlabs` to use the voice relay at the origin of `ANTHROPIC_BASE_URL`, authenticated with `ANTHROPIC_AUTH_TOKEN` (falling back to `ANTHROPIC_API_KEY`, then `CLAUDE_CODE_OAUTH_TOKEN`). The provider service must implement `/v1/voice/elevenlabs/` and have an active ElevenLabs API-key connection for that user. Select `upstream:gemini` or `upstream:paxalabs` for their corresponding relay routes and connected credentials. A messages-only provider does not support this relay. Direct ElevenLabs remains available by selecting `elevenlabs` and setting `ELEVENLABS_API_KEY`. Models and voice IDs retain their ElevenLabs identifiers; the voice/model catalog is obtained through the selected connection. No provider API key is sent to the browser.

### Setting up voice providers

The routes below describe the adapters implemented in this gateway. Choose an available model from **Agents → Voice**; model access and quotas depend on the connected provider account. `provider` selects the connection and billing route; `model` selects the model within that connection.

| Provider | Direct `provider` value | Direct API-key variable | Through upstream | Browser STT in this gateway | Uploaded voice notes |
|---|---|---|---|---|---|
| ElevenLabs | `elevenlabs` | `ELEVENLABS_API_KEY` | `upstream:elevenlabs` | Realtime partial transcripts | Supported with a file STT model such as `scribe_v2` |
| Gemini | `gemini` | `GEMINI_API_KEY` | `upstream:gemini` | Recorded segments, **not Gemini Live** | Supported |
| PaxaLabs | `paxalabs` | `PAXALABS_API_KEY` | `upstream:paxalabs` | Recorded segments, **not realtime partials** | Supported; Thai/English |
| Deepgram | `deepgram` | `DEEPGRAM_API_KEY` | Not implemented | Realtime STT only | Not implemented |
| Cartesia | `cartesia` | `CARTESIA_API_KEY` | Not implemented | No STT adapter; TTS only | Use another provider for notes |

For ElevenLabs, use **`upstream:elevenlabs`**. The previous `upstream` spelling remains a supported alias; configuration migration and new saves use the canonical name while preserving models and voice IDs. There is no automatic switch between direct and upstream credentials if a request fails. You can mix providers: for example, ElevenLabs realtime STT with Gemini TTS.

**1. Add credentials for direct connections.** On the machine running the gateway, edit `~/.claude-gateway/.env` (for this installation: `/home/getpod/.claude-gateway/.env`). Add only the providers you use:

```dotenv
ELEVENLABS_API_KEY=your_elevenlabs_api_key
GEMINI_API_KEY=your_gemini_api_key
PAXALABS_API_KEY=your_paxalabs_api_key
# Optional adapters:
DEEPGRAM_API_KEY=your_deepgram_api_key
CARTESIA_API_KEY=your_cartesia_api_key
```

Keep API keys out of `config.json` and Git. Restart the gateway after changing environment variables; a config hot reload does not reload this global `.env` file. Variables already exported by the shell/service take precedence over this file. These direct-provider keys are gateway-process credentials, not separate API keys per Agent.

**2. Configure the Agent.** Keep `gateway.orchestration: true` and place `voice` directly inside the desired Agent in `agents[]`, alongside its `id`, not inside `orchestration`. `tts` controls the Agent's spoken reply, `stt` controls browser microphone transcription, and `notes` controls uploaded voice messages. Example direct connections:
...
"voice": {
  "enabled": true,
  "tts": { "provider": "gemini", "model": "gemini-2.5-flash-preview-tts", "voiceId": "Kore" },
...
"voice": {
  "enabled": true,
  "tts": { "provider": "paxalabs", "model": "paxa-tts-flash-v1", "voiceId": "" },
  "stt": { "provider": "paxalabs", "model": "paxa-stt-lite-v1-preview" },
  "notes": { "enabled": true, "provider": "paxalabs", "model": "paxa-stt-lite-v1-preview" }
}
```

These are JSON property excerpts, not complete configuration files. Model IDs are examples; select a replacement from your provider catalog if an example is unavailable for your account. An empty `voiceId` requests a stable default from the connected voice catalog; select a specific voice in the UI or with `/voices` to pin it. Catalog access is still required for the automatic default. Gemini and PaxaLabs STT return text **after a recorded segment is committed**, so the browser does not display live word suggestions for those adapters. The PaxaLabs integration is limited to Thai and English. Install `ffmpeg` in the gateway environment for PaxaLabs browser audio, Gemini channel audio, and LINE audio conversion.

**3. Or use the upstream connection.** Connect the desired voice provider/API key in your upstream provider service (for GetPod, use **Providers**), then change each chosen `provider` to `upstream:elevenlabs`, `upstream:gemini`, or `upstream:paxalabs`. Keep that provider's model/voice IDs. The gateway uses the existing `ANTHROPIC_BASE_URL` and upstream credentials described above; the direct `ELEVENLABS_API_KEY`, `GEMINI_API_KEY`, or `PAXALABS_API_KEY` is not needed for an upstream route. Claude settings environment overrides inherited environment for the upstream connection. The service must implement the matching `/v1/voice/<provider>/` endpoints; supporting chat models alone is insufficient.

Bare model IDs and matching catalog IDs such as `elevenlabs/eleven_flash_v2_5` are accepted on their selected route. A `provider/model` prefix does not itself enable upstream routing or choose a managed pool.

**Managed voice credits.** Provider services supporting a managed voice pool can expose `managed:elevenlabs` or `managed:paxalabs`, with catalog IDs `getpod-voice/<provider>/<native-model>`. These routes reuse the upstream credentials above and bill the managed wallet; they are separate from direct and upstream BYOK routes. When the wallet is exhausted, channel voice notes and TTS pause quietly while text chat remains available. Browser ticket creation reports `MANAGED_VOICE_QUOTA_EXHAUSTED` so the client can pause voice until reset. Saved settings are preserved and each new voice request checks the wallet again. Other provider failures retain their diagnostics; missing BYOK credentials never trigger managed billing automatically.

**4. Choose when to reply with audio.** Enabling Agent voice permits voice features; it does not turn on voice replies in every chat. In supported chat channels, use `/voice`: **Always** replies with audio to text and voice inputs; **Only reply voice message** limits audio to replies originating from voice input; **Off** disables voice replies. `/voices` chooses the chat's voice. For background task results, the latest task instruction—including updates and answers—determines whether the originating input was voice. Incoming voice-note transcription remains controlled separately by `notes.enabled`. Browser audio requires an active voice connection. Voice setting changes apply to the next voice connection and subsequent replies.
...
**Experimental conversation intake.** Configure `agents[].orchestration.conversation.semanticIntake: true` to enable preparation of incomplete materials before a complete instruction arrives. `intakeWaitMs` in that same object defaults to **2000 ms** and controls the silence interval before a clarification. Further material restarts the interval; a complete instruction does not wait for this timer. This setting is separate from voice transcription’s `silenceCommitMs`. Greetings and directly answerable conversational questions use one normal reply, without a separate task acknowledgement. The interval starts from the latest received material; model preparation may take longer, and recording/upload-in-progress signals are not included. A direct answer consumes the prepared context. A task acknowledgement does not consume pending reports from earlier tasks; those remain available for the next reporting turn.

Worker results reach the reporting Agent in full. To avoid repeatedly spending tokens on old reports, unrelated historical tasks appear as a status index with a reference to their complete stored result (`task_status` with `task_id`). Current reports are not sliced or summarized by the gateway. The existing managed-turn text limit still applies; oversized output fails explicitly rather than appearing successful with missing text.
...
**Task CLI.** With a running orchestration gateway, use `claude-gateway tasks list --agent <id> --session <id>`; add `--all` for finished tasks, or `--page <n> --page-size <n>` for pagination. `tasks show <task-id>` returns the full retained result. `tasks watch [task-id]` polls every three seconds and prints changes; watching one task ends when it reaches a terminal state. Ctrl+C stops the watcher, not the task. `tasks cancel <task-id>` requests cancellation as the user through the running gateway, so execution fencing and Agent notifications are preserved. All commands accept `--agent`, `--session`, and the usual `--url`, `--key`, `--config`, `--json` options. Agent access and conversation membership are required; this does not bypass channel/session ownership or work offline against the database. API settings and saved data remain unchanged in legacy mode; Task endpoints return `409 ORCHESTRATION_DISABLED`.
...
Active workers receive `when_ready` task updates through scoped Claude Code tool-boundary hooks, in the same CLI session. Delivery is acknowledged separately from acceptance; an update arriving during a long tool waits for that tool to return. A Stop hook handles a pending update before completion. Container workers use only their existing container-local task bridge. Host workers preserve user hook settings; if the operator disables all hooks, pending updates retain the existing end-of-attempt fallback.

Worker supervision is advisory: `tasks.progressStaleMs` (default 180000) requests fresh evidence when no new progress report arrives, even if CPU/tools remain active. `tasks.repeatedToolThreshold` (default 6, range 1–64) flags identical tool inputs and results within the most recent 64 results; raw tool data is not retained by the detector. Normal polling can trigger an advisory, so this is not proof of a stall. Internal inspection uses `tasks.progressNotifyCooldownMs` (default 300000, five minutes), independently of user-facing reporting. The agent checks persisted progress and can append scoped planning advice. It sends a message only for meaningful new progress, changed plans, blockers or needed user decisions; otherwise it completes the review silently, without chat history or TTS. Recent communicated messages are supplied for semantic deduplication, with an additional exact-text guard. Completion, failure, cancellation and input requests continue through their normal event notifications. A new worker progress report does not revoke an assigned advisory; advice remains limited to one update for that alert and execution attempt. These settings never impose a task deadline.

Workers can attach a structured `checkpoint` to `task_report_progress`: current phase, evidence version, completed checks, unresolved/resolved findings, next action and review identity. `task_status` exposes the latest checkpoint; it is reported evidence, not independently verified success. Repeated full reviews without a stated new risk or user request return an advisory warning. The runtime instructions prefer verifying affected behavior after fixes and reusing checks only while their relevant evidence is unchanged; required skill checks still apply.

Supervision advice records its execution attempt and evidence version. If worker evidence advances before delivery, the stale advice is omitted. Advice alone cannot block a worker's final Stop hook; pending user amendments still must be delivered. These safeguards reduce repeated review/fix loops without imposing an execution deadline or claiming to determine semantic completion automatically.


Task details expose a measured time breakdown for working/model time, tool execution/waiting, waiting for user input, queued time, and recovery/stopping. Tool time includes execution and external waits such as CI; it does not claim CPU time or verified progress. Existing tasks begin measurement after upgrade; unrecorded historical phases are not guessed.
...
Voice latency diagnostics: live voice logs `Voice timing` events with session, operation (`stt`/`tts`), and operation ID. TTS phases distinguish speech readiness, synthesis start, HTTP request/headers, first provider audio, first audio sent, and the first positive playback receipt. `elapsedMs` is measured from speech readiness (including queue wait) or STT commit; subtract phase times to measure individual stages. HTTP phases apply to providers using the HTTP request helper; Gemini also records full response-body receipt. The first playback receipt is an observation from the client, not an exact audio-device start timestamp. Audio completion includes playback backpressure and must not be treated as time to first audio. Logs exclude transcripts, audio, URLs, and credentials. Gemini HTTP failures identify STT or TTS separately.

### Reply context and channel attachments

Orchestration passes quoted-message context to both the user-facing agent and its workers, including buffered inputs and follow-ups. Quotes are reference data, not fresh authorization. Telegram preserves quoted captions and downloads quoted files; Discord retains multiple files and same-channel message references; Slack supports general files alongside images/audio and retrieves the referenced thread root when its token has permission. LINE and WhatsApp Cloud provide quote IDs, so missing content is resolved only from this account/chat's authorized local history. If it is unavailable, the agent is told explicitly rather than guessing. Linked WhatsApp supports document/audio/video attachments and quoted media; WeChat preserves native `ref_msg` text and supported image/file references. Historical quote metadata is also retained for chat history.

Provider limits still apply. LINE delivers ordinary files as expiring download links (images/audio use native message types). Slack thread retrieval depends on history permissions and API availability. Native attachment uploads remain scoped to the original channel/chat/thread; uncertain delivery receipts are not blindly retried.
