...
**Knowledge archive (`gateway.knowledge.archive`).** Two-lane memory, phase K0 (planning-64). A per-agent SQLite/FTS5 index (`agents/<id>/kb.sqlite`, built on `node:sqlite` — zero new dependency) over the agent's `memory/*.md` notes plus the evergreen `MEMORY.md`/`USER.md`. Injected by migration at `configVersion` `1.0.21`. Indexing is **hash-guarded** (unchanged files skipped), tags every chunk with **fail-closed provenance** (`owner`/`agent`/`untrusted`/`system`; unclassified ⇒ `untrusted`), and prunes sources whose file disappeared. This phase is **dormant infrastructure**: building the index changes nothing about the injected prompt and adds no tools yet (on-demand retrieval + core-shrink are later phases). All indexing runs off the gateway event loop. Fields: `enabled` (default `true`; `false` ⇒ complete no-op, no DB created), `tokenizer` (FTS5 tokenizer, default `"unicode61"`; `"trigram"` for CJK/Thai), `chunkTokens` (default `400`), `chunkOverlap` (default `80`, clamped below `chunkTokens`). Per-agent override under `agents[].knowledge`.
