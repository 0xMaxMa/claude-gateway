...
// live channels — derived here so a future 5th channel only needs adding to
// CHAT_CHANNELS, not a second hardcoded list.
const SESSION_DIR_PREFIXES = [...CHAT_CHANNELS, 'api'] as const;
...
export class SessionStore {
   ...
   */
  private resolveTelegramDir(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): string {
   ...
   */
  private resolveTelegramIndexPath(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): string {
   ...
   */
  private resolveTelegramSessionPath(agentId: string, chatId: string, sessionId: string, channel: ChatChannelOrApi = 'telegram'): string {
   ...
   */
  async loadIndex(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): Promise<SessionIndex | null> {
   ...
   */
  async saveIndex(agentId: string, chatId: string, index: SessionIndex, channel: ChatChannelOrApi = 'telegram'): Promise<void> {
    const indexPath = this.resolveTelegramIndexPath(agentId, chatId, channel);
   ...
   */
  private async loadOrCreateIndexUnlocked(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): Promise<SessionIndex> {
  ...
  async getOrCreateIndex(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): Promise<SessionIndex> {
   ...
   */
  async createTelegramSession(agentId: string, chatId: string, name?: string, channel: ChatChannelOrApi = 'telegram'): Promise<SessionMeta> {
    const queue = this.getTelegramQueue(agentId, chatId);
  ...
  async listSessions(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): Promise<SessionIndex> {
    return this.getOrCreateIndex(agentId, chatId, channel);
  }

  async getActiveSessionId(agentId: string, chatId: string, channel: ChatChannelOrApi = 'telegram'): Promise<string> {
    const index = await this.getOrCreateIndex(agentId, chatId, channel);
    return index.activeSessionId;
  }

  async setActiveSession(agentId: string, chatId: string, sessionId: string, channel: ChatChannelOrApi = 'telegram'): Promise<void> {
    const queue = this.getTelegramQueue(agentId, chatId);
    await queue.add(async () => {
  ...
  async deleteTelegramSession(agentId: string, chatId: string, sessionId: string, channel: ChatChannelOrApi = 'telegram'): Promise<void> {
    const queue = this.getTelegramQueue(agentId, chatId);
    await queue.add(async () => {
      const index = await this.loadIndex(agentId, chatId, channel);
      if (!index) {
  ...
  async clearTelegramSessionHistory(agentId: string, chatId: string, sessionId: string, channel: ChatChannelOrApi = 'telegram'): Promise<void> {
  ...
  async loadTelegramSession(agentId: string, chatId: string, sessionId: string, channel: ChatChannelOrApi = 'telegram'): Promise<Message[]> {
  ...
  async getAllSessionMeta(agentId: string): Promise<Map<string, { name: string; imageConfig?: ImageParams; model?: string }>> {
    ...
    const reads = entries
      .filter((e) => e.isDirectory() && SESSION_DIR_PREFIXES.some((c) => e.name.startsWith(`${c}-`)))
      .map(async (e) => {
        const channel = SESSION_DIR_PREFIXES.find((c) => e.name.startsWith(`${c}-`)) ?? 'telegram';
