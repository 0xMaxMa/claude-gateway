...
function requestDetails(requests: TokenReportView['turns'][number]['requests'], aggregate?: TokenReportView['turns'][number]['usage']): string {
  ...
  return `<div class="request-details"><h3>Observed model requests: ${requests.length}</h3>${incomplete ? '<p class="muted">Observed requests do not cover the full turn aggregate; additional request usage was not reported separately.</p>' : ''}<table><thead><tr><th>Request / model</th><th title="Uncached input only. Total input = fresh input + cache write + cache read.">Fresh input</th><th>Cache write</th><th>Cache read</th><th>Output</th><th>Total</th></tr></thead><tbody>${requests.map(request => `<tr><td>${escape(request.id)}<br>${escape(request.model ?? '—')}</td><td>${n(request.usage.inputTokens)}</td><td>${cacheWrite(request.usage)}</td><td>${n(request.usage.cacheReadTokens)}</td><td>${n(request.usage.outputTokens)}</td><td>${n(request.usage.totalTokens)}</td></tr>`).join('')}</tbody></table></div>`;
}
...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const rows=turns.map((turn,index)=>{
  ...
  return `<tr class="turn-row" tabindex="0" aria-expanded="false" data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td><strong>${escape(number)}</strong><small>${escape(time(turn.startedAt))}</small><small class="cell-clip" title="${escape(turn.model)}">${escape(turn.model||'—')}</small></td><td><span class="turn-kind ${escape(turn.category)}">${escape(badge)}</span><strong class="turn-excerpt" title="${escape(title.slice(0,1000))}">${escape(title.slice(0,180))}${title.length>180?'…':''}</strong>${turn.state?statusBadge(turn.state):''}</td><td>${n(turn.usage?.inputTokens)}</td><td>${cacheWrite(turn.usage)}</td><td>${n(turn.usage?.cacheReadTokens)}</td><td>${n(turn.usage?.outputTokens)}</td><td><strong>${n(turn.usage?.totalTokens)}</strong></td><td>${tools('Loaded',turn.contextTools)}${tools('Used',turn.usedTools)}</td></tr><tr class="turn-extra" hidden data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td colspan="8"><h3>Conversation & request details</h3><p class="muted">Role: ${escape(turn.role)} · Turn ${escape(turn.id)}${turn.taskId?' · Task '+escape(turn.taskId):''}${turn.endedAt?' · Ended '+escape(time(turn.endedAt)):''}</p>${conversationDetails(turn)}${requestDetails(turn.requests,turn.usage)}${turn.contextTools?'<h3>Loaded tools</h3>'+toolNameList(turn.contextTools):''}<h3>Used tools</h3>${toolNameList(turn.usedTools)}<p class="muted">Loaded counts distinct schemas observed in requests, including referenced deferred tools. Used counts distinct tool names executed. ${turn.schemaCoverage?`Schema capture: ${turn.schemaCoverage.measured}/${turn.schemaCoverage.total} observed requests.`:'Schema capture was not recorded for this turn.'}</p></td></tr>`;
 }).join('');
