...
function conversationDetails(turn: TokenReportView['turns'][number]): string {
  ...
  return `<div class="conversation-details"><h3>Conversation / assignment / result</h3>${turn.state ? '<p>Status: ' + escape(turn.state) + '</p>' : ''}${assignment}${inputs ? '<h3>Input</h3>' + inputs : ''}${response}</div>`;
}
