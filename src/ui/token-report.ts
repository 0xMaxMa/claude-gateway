...
import { agentBadge, channelBadge, compactNumber } from './dashboard-presentation';
...
function statusBadge(state: string): string {
 const success=['completed','succeeded','success'].includes(state);
 const color=success?'badge-green':state==='failed'?'badge-red':state==='running'||state==='working'?'badge-blue':'badge-gray';
 const label=success?'Success':state.replace(/_/g,' ').replace(/^./,c=>c.toUpperCase());
 return '<span class="badge '+color+'" title="'+escape(state)+'">'+escape(label)+'</span>';
}
function conversationDetails(turn: TokenReportView['turns'][number]): string {
  ...
  return `<div class="conversation-details"><h3>Conversation / assignment / result</h3>${turn.state ? '<p class="turn-status"><span>Status</span> ' + statusBadge(turn.state) + '</p>' : ''}${assignment}${inputs ? '<h3>Input</h3>' + inputs : ''}${response}</div>`;
}
...
function footprintHtml(footprint?: ContextFootprint): string {
 ...
 const items=parts.map(r=>({name:r.name.replace(/^↳ /,''),tokens:r.tokens!,color:color(r.name)}));
 return `<section id="report-footprint"><div class="report-toolbar"><h2 style="margin:0">Current context footprint</h2><strong style="margin-left:auto">${total?n(total)+' estimated tokens':'Unavailable'}</strong></div>${total?`<div class="stacked-distribution" aria-label="Estimated context tokens by source">${items.map(r=>`<span class="segment" style="background:${r.color};width:${r.tokens/total*100}%" title="${escape(r.name)}: ${n(r.tokens)} tokens"></span>`).join('')}</div><div class="distribution-legend">${items.map(r=>`<div class="distribution-item"><i style="background:${r.color}"></i><span>${escape(r.name)}</span><strong>${n(r.tokens)}</strong></div>`).join('')}</div>`:'<p class="muted">Current workspace footprint unavailable.</p>'}<p class="muted">Sum of non-empty composed sections and Agent gateway tool schemas; source files are not counted again. Estimated with cl100k_base, not historical turn attribution or the selected model’s exact token count. Excludes section headers, conversation history, runtime overlays, CLI instructions/tools and provider framing. ${footprint?'Checked '+escape(time(footprint.observedAt))+' · refreshes every 30s.':''}</p></section>`;
}
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const rows=turns.map((turn,index)=>{
  ...
  return `<tr class="turn-row" data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td><span class="turn-kind ${escape(turn.category)}">${escape(badge)}</span><strong class="turn-excerpt" title="${escape(title.slice(0,1000))}">${escape(title.slice(0,180))}${title.length>180?'…':''}</strong><small>${escape(number)} · ${escape(time(turn.startedAt))}</small><small class="cell-clip" title="${escape(turn.model)}">${escape(turn.model||'Model unavailable')}</small></td><td>${n(turn.usage?.inputTokens)}</td><td>${n(turn.usage?.cacheCreationTokens)}<small>5m: ${n(turn.usage?.cacheCreation5mTokens)}<br>1h: ${n(turn.usage?.cacheCreation1hTokens)}</small></td><td>${n(turn.usage?.cacheReadTokens)}</td><td>${n(turn.usage?.outputTokens)}</td><td><strong>${n(turn.usage?.totalTokens)}</strong></td><td>${tools('Loaded',turn.loadedTools)}${tools('Used',turn.usedTools)}</td></tr><tr class="turn-extra" data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td colspan="7"><details><summary>Conversation & request details</summary><p class="muted">Role: ${escape(turn.role)} · Turn ${escape(turn.id)}${turn.taskId?' · Task '+escape(turn.taskId):''}${turn.endedAt?' · Ended '+escape(time(turn.endedAt)):''}</p>${conversationDetails(turn)}${requestDetails(turn.requests,turn.usage)}</details></td></tr>`;
 }).join('');
