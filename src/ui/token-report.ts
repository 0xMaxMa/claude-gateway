import { codexContextHtml } from './codex-context';
import type { CodexContextMeasurement } from '../session/codex-context';
import { themeButtonHtml, dashboardControlsClient } from './dashboard-controls';
...
import { dashboardChartPalette, agentBadge, channelBadge, compactNumber, taskStatusBadge, toolNameList, providerWaitingHtml } from './dashboard-presentation';
...
export interface TokenReportView {
  ...
  providerWaiting?: {state: string; reason: string; nextRetryAt?: number; requiresConfigurationChange: boolean};
  scope?: string;
  ...
  turns: Array<{
    ...
    id: string; role: string; category: string; taskId?: string; harness?: 'claude' | 'codex';
    ...
    attemptGeneration?: number; pending?: boolean; queuedMs?: number;
...
function tools(label: string, names: string[] | null | undefined, harness?: string): string {
  return `<div title="${escape(names?.map(name=>harness==='codex'&&name==='Bash'?'Shell':name).join(', ')||'Not recorded')}">${label}: ${names == null ? '—' : n(names.length)}</div>`;
}
...
function waited(ms: number | undefined): string {
...
/** Wall time for this turn only; queued inputs have not started execution. */
function turnElapsed(turn: TokenReportView['turns'][number]): string {
 if(turn.pending)return '—';
 const start=new Date(turn.startedAt).getTime();
 const ended=turn.endedAt!==undefined;
 if(!ended&&['completed','failed','cancelled','stopped','ended'].includes(turn.state||''))return '—';
 const end=ended?new Date(turn.endedAt!).getTime():Date.now();
 return waited(end-start)||'—';
}
...
function turnLabel(turn: TokenReportView['turns'][number]): string {
 if(turn.inputSequences?.length)return 'Input #'+turn.inputSequences.join(', #');
 if(typeof turn.attemptGeneration==='number'&&Number.isFinite(turn.attemptGeneration))return 'Attempt #'+turn.attemptGeneration;
 return 'Unnumbered turn';
}
function time(value: string | number, timezone = 'UTC'): string {
 const date=new Date(value);return Number.isFinite(date.getTime()) ? date.toLocaleString('en-GB',{timeZone:timezone,day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit',second:'2-digit'})+' '+timezone : '—';
}
...
function relativeTime(value: string | number, timezone = 'UTC'): { label: string; title: string } {
...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const cachedLineFor=(u?:{inputTokens:number;cacheReadTokens:number;cacheCreationTokens:number})=>u?`<span class="card-cached">Cached: <b>${cachedRatio(u.inputTokens,u.cacheReadTokens,u.cacheCreationTokens).toFixed(2)}%</b></span>`:'';
 ...
 const rows=turns.map(turn=>{
  ...
  // Show live queue duration only while pending; historical wait belongs in details.
  const wait=waited(turn.queuedMs);
  const waitLine=turn.pending
   ? `<small class="turn-wait" title="Accepted ${escape(time(turn.startedAt,timezone))}; waiting for the previous turn in this session to finish">Queued${wait?' · '+escape(wait):''}</small>`
   : '';
  ...
  return `<tr class="turn-row${turn.pending?' turn-pending':''}" tabindex="0" aria-expanded="false" data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}" data-model="${escape(turn.model||'')}" data-status="${escape(turn.state||'')}"><td><strong>${escape(number)}</strong><small title="${escape(started.title)}">${escape(started.label)}</small>${waitLine}<small class="cell-model" title="${escape(turn.model)}">${escape(turn.model||'—')}</small></td><td><span class="turn-kind ${escape(turn.category)}">${escape(badge)}</span><strong class="turn-excerpt" title="${escape(title.slice(0,1000))}">${escape(title.slice(0,180))}${title.length>180?'…':''}</strong>${turn.state?statusBadge(turn.state):''}</td><td>${n(turn.usage?.inputTokens)}</td><td>${cacheWrite(turn.usage)}</td><td>${n(turn.usage?.cacheReadTokens)}</td><td>${n(turn.usage?.outputTokens)}</td><td><strong>${n(turn.usage?.totalTokens)}</strong></td><td>${turn.usage?'<strong>'+cachedRatio(turn.usage.inputTokens,turn.usage.cacheReadTokens,turn.usage.cacheCreationTokens).toFixed(2)+'%</strong>':'—'}</td><td>${tools('Loaded',turn.contextTools)}${tools('Used',turn.usedTools,turn.harness)}</td></tr><tr class="turn-extra" hidden data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td colspan="9"><h3>Conversation & request details</h3><div class="turn-metadata muted"><div>Role: ${escape(turn.role)}</div>${turn.role === 'worker' ? '<div>Harness: ' + (turn.harness ? '<span class="harness-badge ' + (turn.harness === 'codex' ? 'harness-codex' : 'harness-claude') + '">' + (turn.harness === 'codex' ? 'Codex' : 'Claude Code') + '</span>' : '—') + '</div>' : ''}<div>Model: ${escape(turn.model || '—')}</div><div>Turn ${escape(turn.id)}</div>${turn.taskId?'<div>Task '+escape(turn.taskId)+'</div>':''}${wait?'<div>'+escape(turn.pending?'Waiting '+wait:'Waited '+wait+' before starting')+'</div>':''}<div>${turn.pending?'Queued since':'Started'} ${escape(time(turn.startedAt,timezone))}</div><div>Elapsed: ${escape(turnElapsed(turn))}</div></div>${pendingNote}${codexContextHtml(turn.contextWindow)}${conversationDetails(turn)}${requestDetails(turn.requests,turn.usage)}${turn.contextTools?'<h3>Loaded tools</h3>'+toolNameList(turn.contextTools):''}<h3>Used tools</h3>${toolNameList(turn.usedTools,turn.harness)}<p class="muted">Loaded counts distinct schemas observed in requests, including referenced deferred tools. Used counts distinct tool names executed. ${turn.schemaCoverage?`Schema capture: ${turn.schemaCoverage.measured}/${turn.schemaCoverage.total} observed requests.`:'Schema capture was not recorded for this turn.'}</p></td></tr>`;
 }).join('');
 return `<!DOCTYPE html><html lang="en" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Session token report · Claude Gateway</title>${dashboardFontLink}<style>.harness-badge{display:inline-flex;align-items:center;padding:2px 7px;border-radius:6px;font-size:11px;line-height:1.4;font-weight:500;vertical-align:middle}.harness-codex{background:var(--bluebg);color:var(--blue)}.harness-claude{background:var(--yellowbg);color:var(--yellow)}.turn-metadata{display:grid;gap:6px;margin:12px 0 18px;overflow-wrap:anywhere}${dashboardTheme}
