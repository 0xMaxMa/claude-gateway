...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
  ...
  const measured = report.pagination ? report.totals.totalTokens !== null : report.turns.some(turn=>turn.usage);
  const roleMeasured = (role: string) => report.pagination ? (role === 'agent' ? report.totals.agentTokens !== null : report.totals.workerTokens !== null) : report.turns.some(turn=>turn.role===role&&turn.usage);
  ...
  const rows = report.turns.map((turn, index) => `<tr><td>${(report.pagination?.offset ?? 0) + index + 1}<br><span class="muted">${escape(turn.id)}</span></td><td>${escape(labels[turn.category] ?? turn.category)}<br><span class="muted">Role: ${escape(turn.role)}<br>${escape(turn.model ?? 'Model unavailable')}<br>${escape(typeof turn.startedAt === 'number' ? new Date(turn.startedAt).toISOString() : turn.startedAt)}${turn.endedAt ? '<br>Ended: ' + escape(typeof turn.endedAt === 'number' ? new Date(turn.endedAt).toISOString() : turn.endedAt) : ''}${turn.taskId ? '<br>Task: ' + escape(turn.taskId) : ''}</span></td><td>${n(turn.usage?.inputTokens)}</td><td>${n(turn.usage?.cacheCreationTokens)}<br><span class="muted">5m: ${turn.usage?.cacheCreation5mTokens === undefined ? 'Unavailable' : n(turn.usage?.cacheCreation5mTokens)}<br>1h: ${turn.usage?.cacheCreation1hTokens === undefined ? 'Unavailable' : n(turn.usage?.cacheCreation1hTokens)}</span></td><td>${n(turn.usage?.cacheReadTokens)}</td><td>${n(turn.usage?.outputTokens)}</td><td>${n(turn.usage?.totalTokens)}</td><td>${tools('Loaded', turn.loadedTools)}${tools('Used', turn.usedTools)}</td></tr><tr><td colspan="8">${conversationDetails(turn)}${requestDetails(turn.requests, turn.usage)}</td></tr>`).join('');
  const page = report.pagination;
  const link = (offset:number) => '?agentId='+encodeURIComponent(agentId)+'&sessionId='+encodeURIComponent(report.sessionId)+'&offset='+offset;
  const pager = page ? `<nav class="report-pager" aria-label="Report pages">${page.offset?`<a href="${escape(link(Math.max(0,page.offset-page.limit)))}">← Previous turns</a>`:''}<span>Turns ${page.total? page.offset+1:0}–${Math.min(page.offset+page.limit,page.total)} of ${page.total}. Totals cover the complete recorded session.</span>${page.offset+page.limit<page.total?`<a href="${escape(link(page.offset+page.limit))}">Next turns →</a>`:''}</nav>`:'';
  return `<!DOCTYPE html><html lang="en" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Session token report · Claude Gateway</title><style>
...
${dashboardTheme}
body{padding:32px;background:var(--bg);color:var(--text)}main{max-width:1500px;margin:auto}h1{font-size:36px;margin:28px 0 16px}h2{color:var(--text)}h3{color:var(--muted)}section,.card{border:1px solid var(--line);border-radius:24px;background:var(--panel);padding:24px}.cards .card:nth-child(1){background:var(--mint)}.cards .card:nth-child(2){background:var(--sky)}.cards .card:nth-child(3){background:var(--lavender)}.muted,th{color:var(--muted)}td,th{border-color:var(--line)}pre{background:var(--raised);font-size:12px}a{color:var(--accent)}progress{accent-color:var(--accent)}.report-pager{display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;padding:20px 0;font-size:12px}.distribution strong{font-size:14px}.card strong{font-size:32px;font-weight:400}@media(max-width:760px){body{padding:20px 14px}.distribution{grid-template-columns:1fr}.table-scroll{max-width:100%}section{padding:20px}}
</style>${dashboardFontLink}</head><body><main><a href="../dashboard">← Dashboard</a><h1>Session token report</h1><p class="muted">Agent: ${escape(agentId)} · Session: ${escape(report.sessionId)}</p><div class="cards"><div class="card">Agent tokens<strong>${roleMeasured('agent') ? n(report.totals.agentTokens) : 'Unavailable'}</strong></div><div class="card">Worker tokens<strong>${roleMeasured('worker') ? n(report.totals.workerTokens) : 'Unavailable'}</strong></div><div class="card">Combined tokens<strong>${measured ? n(report.totals.totalTokens) : 'Unavailable'}</strong></div></div><p class="muted">Recorded turns only. Older turns without instrumentation are unavailable. Token volume includes fresh input, cache creation, cache reads and output; these percentages are not monetary costs. Thinking is included in output, never added twice. Background learning is shown separately when recorded.</p><section><h2>Token distribution</h2>${distribution || '<p>No recorded token usage.</p>'}</section><section><h2>Turn details</h2>${pager}<div class="table-scroll"><table><thead><tr><th>Turn</th><th>Purpose / model / task</th><th>Fresh input</th><th>Cache creation</th><th>Cache reads</th><th>Output</th><th>Total</th><th>Tools</th></tr></thead><tbody>${rows || '<tr><td colspan="8">No recorded turns available.</td></tr>'}</tbody></table></div></section>${pager}${backgroundDetails(report.backgroundReviews)}</main></body></html>`;
}
