...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const rows=turns.map(turn=>{
  ...
  return `<tr class="turn-row${turn.pending?' turn-pending':''}" tabindex="0" aria-expanded="false" data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}" data-model="${escape(turn.model||'')}" data-status="${escape(turn.state||'')}"><td><strong>${escape(number)}</strong><small title="${escape(started.title)}">${escape(started.label)}</small>${waitLine}<small class="cell-model" title="${escape(turn.model)}">${escape(turn.model||'—')}</small></td><td><span class="turn-kind ${escape(turn.category)}">${escape(badge)}</span><strong class="turn-excerpt" title="${escape(title.slice(0,1000))}">${escape(title.slice(0,180))}${title.length>180?'…':''}</strong>${turn.state?statusBadge(turn.state):''}</td><td>${n(turn.usage?.inputTokens)}</td><td>${cacheWrite(turn.usage)}</td><td>${n(turn.usage?.cacheReadTokens)}</td><td>${n(turn.usage?.outputTokens)}</td><td><strong>${n(turn.usage?.totalTokens)}</strong></td><td>${turn.usage?'<strong>'+cachedRatio(turn.usage.inputTokens,turn.usage.cacheReadTokens,turn.usage.cacheCreationTokens).toFixed(2)+'%</strong>':'—'}</td><td>${tools('Loaded',turn.contextTools)}${tools('Used',turn.usedTools)}</td></tr><tr class="turn-extra" hidden data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td colspan="9"><h3>Conversation & request details</h3><p class="muted">Role: ${escape(turn.role)} · Turn ${escape(turn.id)}${turn.taskId?' · Task '+escape(turn.taskId):''}${turn.endedAt?' · Ended '+escape(time(turn.endedAt)):''}${wait?' · '+escape(turn.pending?'Waiting '+wait:'Queued '+wait+' before starting'):''}</p>${pendingNote}${conversationDetails(turn)}${requestDetails(turn.requests,turn.usage)}${turn.contextTools?'<h3>Loaded tools</h3>'+toolNameList(turn.contextTools):''}<h3>Used tools</h3>${toolNameList(turn.usedTools)}<p class="muted">Loaded counts distinct schemas observed in requests, including referenced deferred tools. Used counts distinct tool names executed. ${turn.schemaCoverage?`Schema capture: ${turn.schemaCoverage.measured}/${turn.schemaCoverage.total} observed requests.`:'Schema capture was not recorded for this turn.'}</p></td></tr>`;
 }).join('');
