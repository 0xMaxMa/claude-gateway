...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const rows=turns.map(turn=>{
  ...
  const waitLine=turn.pending
   ? `<small class="turn-wait" title="Accepted ${escape(time(turn.startedAt,timezone))}; waiting for the previous turn in this session to finish">Queued · ${escape(wait||'0s')}</small>`
   : '';
