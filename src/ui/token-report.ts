...
export interface TokenReportView {
  ...
  // session kept alive, 'stopped' = killed/removed. Display-only.
...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const cachedLineFor=(u?:{inputTokens:number;cacheReadTokens:number;cacheCreationTokens:number})=>u?`<span class="card-cached">Cached: <b>${n(u.cacheReadTokens)} · ${cachedRatio(u.inputTokens,u.cacheReadTokens,u.cacheCreationTokens).toFixed(2)}%</b></span>`:'';
