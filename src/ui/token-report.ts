...
function requestDetails(requests: TokenReportView['turns'][number]['requests'], aggregate?: TokenReportView['turns'][number]['usage']): string {
  ...
  return `<div class="request-details"><h3>Observed model requests: ${requests.length}</h3>${incomplete ? '<p class="muted">Observed requests do not cover the full turn aggregate; additional request usage was not reported separately.</p>' : ''}<table><thead><tr><th>Request / model</th><th>Fresh input</th><th>Cache write</th><th>Cache read</th><th>Output</th><th>Total</th></tr></thead><tbody>${requests.map(request => `<tr><td>${escape(request.id)}<br>${escape(request.model ?? '—')}${request.toolSchemas?'<h4>Loaded tools ('+n(request.toolSchemas.loaded.length)+')</h4>'+toolNameList(request.toolSchemas.loaded):''}</td><td>${n(request.usage.inputTokens)}</td><td>${cacheWrite(request.usage)}</td><td>${n(request.usage.cacheReadTokens)}</td><td>${n(request.usage.outputTokens)}</td><td>${n(request.usage.totalTokens)}</td></tr>`).join('')}</tbody></table></div>`;
}
...
export function generateTokenReportHtml(agentId: string, report: TokenReportView): string {
 ...
 const pager=page?`<nav class="report-pager">${page.offset?`<a href="${escape(link(Math.max(0,page.offset-page.limit)))}">← Previous turns</a>`:''}<span>${page.total?page.offset+1:0}–${Math.min(page.offset+page.limit,page.total)} of ${page.total} recorded turns · Newest first</span>${page.offset+page.limit<page.total?`<a href="${escape(link(page.offset+page.limit))}">Next turns →</a>`:''}</nav>`:'';
 ...
 const rows=turns.map((turn,index)=>{
  ...
  return `<tr class="turn-row" tabindex="0" aria-expanded="false" data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td><strong>${escape(number)}</strong><small>${escape(time(turn.startedAt))}</small><small class="cell-clip" title="${escape(turn.model)}">${escape(turn.model||'—')}</small></td><td><span class="turn-kind ${escape(turn.category)}">${escape(badge)}</span><strong class="turn-excerpt" title="${escape(title.slice(0,1000))}">${escape(title.slice(0,180))}${title.length>180?'…':''}</strong>${turn.state?statusBadge(turn.state):''}</td><td>${turn.role==='agent'?n(turn.usage?.totalTokens):'—'}</td><td>${turn.role==='worker'?n(turn.usage?.totalTokens):'—'}</td><td>${n(turn.usage?.inputTokens)}</td><td>${cacheWrite(turn.usage)}</td><td>${n(turn.usage?.cacheReadTokens)}</td><td>${n(turn.usage?.outputTokens)}</td><td><strong>${n(turn.usage?.totalTokens)}</strong></td><td>${tools('Loaded',turn.contextTools)}${tools('Used',turn.usedTools)}</td></tr><tr class="turn-extra" hidden data-turn-id="${escape(turn.id)}" data-category="${escape(turn.category)}"><td colspan="10"><h3>Conversation & request details</h3><p class="muted">Role: ${escape(turn.role)} · Turn ${escape(turn.id)}${turn.taskId?' · Task '+escape(turn.taskId):''}${turn.endedAt?' · Ended '+escape(time(turn.endedAt)):''}</p>${conversationDetails(turn)}${requestDetails(turn.requests,turn.usage)}${turn.contextTools?'<h3>Loaded tools</h3>'+toolNameList(turn.contextTools):''}<h3>Used tools</h3>${toolNameList(turn.usedTools)}<p class="muted">Loaded counts distinct schemas observed in requests, including referenced deferred tools. Used counts distinct tool names executed. ${turn.schemaCoverage?`Schema capture: ${turn.schemaCoverage.measured}/${turn.schemaCoverage.total} observed requests.`:'Schema capture was not recorded for this turn.'}</p></td></tr>`;
 }).join('');
...
</style></head><body><main><div class="report-toolbar"><a href="../dashboard">← Dashboard</a><span id="report-live" class="muted" role="status">Live · every 5s</span><button id="report-theme" style="margin-left:auto">Light / dark</button></div><h1>Session token report</h1><div class="report-header">${agentBadge(agentId)}${channelBadge(report.source||'')}<span class="muted" title="${escape(report.sessionId)}">Session ${escape(report.sessionId)}</span>${rangeButtons("report-scope",scope)}</div><div class="cards" id="report-totals"><div class="card">Agent tokens<strong>${roleMeasured('agent')?n(report.totals.agentTokens):'—'}</strong></div><div class="card">Worker tokens<strong>${roleMeasured('worker')?n(report.totals.workerTokens):'—'}</strong></div><div class="card">Total tokens<strong>${measured?n(report.totals.totalTokens):'—'}</strong></div></div><p class="muted">${report.since?'Turns started since '+escape(time(report.since))+'.':'All recorded turns.'} Totals include all pages. Token volume includes fresh input, cache creation, cache reads and output; these percentages are not monetary costs. Missing measurements are shown as —. Fresh input is uncached input; cache write creates cached context; cache read reuses context; output includes reported thinking. Agent/Worker columns split Totals by role; do not add them to the input/output columns again.</p>${footprintHtml(report.contextFootprint)}<section id="report-distribution"><h2>Token distribution</h2>${total?`<div class="stacked-distribution">${distribution}</div><div class="distribution-legend">${legend}</div>`:'<p>No recorded token usage.</p>'}</section><section><h2>Turn details</h2><div class="report-toolbar"><select id="turn-category" aria-label="Filter turns on this page"><option value="">All purposes on this page</option><option value="input">User input</option><option value="report">Progress / results</option><option value="worker">Workers</option></select><input id="turn-search" placeholder="Search this page's messages or models" aria-label="Search turns on this page"></div>${pager}<div class="table-scroll"><table class="data-table report-table"><colgroup>${[120,0,76,76,72,148,72,64,72,72].map(width=>width?`<col style="width:${width}px">`:'<col>').join('')}</colgroup><thead><tr><th>Turn / Date</th><th>Conversation / assignment</th><th>Agent tokens</th><th>Worker tokens</th><th>Fresh input</th><th>Cache write</th><th>Cache read</th><th>Output</th><th>Totals</th><th>Tools</th></tr></thead><tbody>${rows||'<tr><td colspan="10">No recorded turns in this scope.</td></tr>'}</tbody></table></div>${pager}</section></main><script>
...
function filter(){var category=document.getElementById('turn-category').value,query=document.getElementById('turn-search').value.toLowerCase();document.querySelectorAll('.turn-row').forEach(function(row){var show=(!category||row.dataset.category===category)&&(!query||(row.textContent+row.nextElementSibling.textContent).toLowerCase().includes(query));row.hidden=!show;row.nextElementSibling.hidden=!show||row.getAttribute('aria-expanded')!=='true';});}
document.addEventListener('click',function(e){var row=e.target.closest('.turn-row');if(!row||e.target.closest('a,button,input,select'))return;var expanded=row.getAttribute('aria-expanded')!=='true';row.setAttribute('aria-expanded',String(expanded));row.nextElementSibling.hidden=!expanded;});
document.addEventListener('keydown',function(e){if((e.key==='Enter'||e.key===' ')&&e.target.matches('.turn-row')){e.preventDefault();e.target.click();}});
