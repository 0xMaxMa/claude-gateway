...
function renderDashboard(data){
  ...
  document.getElementById('overview-attention-panel').hidden=!attention.length;document.getElementById('overview-charts').classList.toggle('no-attention',!attention.length);
  ...
  const hoverHours=hourly.map((h,i)=>{const label=String(i).padStart(2,'0')+':00 · Agent '+compactNumber(h.agent)+' · Workers '+compactNumber(h.worker)+' · Total '+compactNumber(h.agent+h.worker);return '<span class="chart-hour" tabindex="0" role="img" aria-label="'+escHtml(label)+'"><span class="chart-tooltip"><strong>'+String(i).padStart(2,'0')+':00</strong><br>Agent: '+compactNumber(h.agent)+'<br>Workers: '+compactNumber(h.worker)+'<br>Total: '+compactNumber(h.agent+h.worker)+'</span></span>';}).join('');
  document.getElementById('overview-chart').innerHTML=hasUsage?'<div class="token-activity-chart"><svg viewBox="0 0 600 180" preserveAspectRatio="none" role="img" aria-label="Recorded token activity today" style="width:100%;height:180px"><path d="M0 20H600M0 90H600M0 160H600" fill="none" stroke="var(--line)" stroke-dasharray="4 5"/><polyline points="'+points('worker')+'" fill="none" stroke="var(--accent)" stroke-width="2"/><polyline points="'+points('agent')+'" fill="none" stroke="var(--blue)" stroke-width="2"/></svg><div class="chart-hour-grid">'+hoverHours+'</div></div><div class="row between muted small"><span>00:00</span><span>12:00</span><span>23:00</span></div>':'<p class="empty">No recorded token usage today.</p>';
  const agentUsage=agents.map(a=>{const usage=(a.orchestration?.usageToday||[]).reduce((sum,r)=>{if(r.role==='agent'||r.role==='worker')sum[r.role]+=Number(r.tokens||0);return sum;},{agent:0,worker:0});return {id:a.id,...usage,total:usage.agent+usage.worker};}).filter(a=>a.total>0).sort((a,b)=>b.total-a.total||a.id.localeCompare(b.id));
  const maxAgent=Math.max(1,...agentUsage.map(a=>a.total));
  document.getElementById('overview-agent-tokens').innerHTML=agentUsage.length?'<div class="agent-token-bars">'+agentUsage.map(a=>'<div class="agent-token-row">'+agentBadge(a.id)+'<div class="agent-token-track" role="img" tabindex="0" aria-label="'+escHtml(a.id+': Agent '+compactNumber(a.agent)+', Workers '+compactNumber(a.worker)+', Total '+compactNumber(a.total))+'" title="Agent: '+compactNumber(a.agent)+' · Workers: '+compactNumber(a.worker)+'"><span style="width:'+(a.agent/maxAgent*100)+'%;background:var(--blue)"></span><span style="width:'+(a.worker/maxAgent*100)+'%;background:var(--accent)"></span></div><strong>'+compactNumber(a.total)+'</strong></div>').join('')+'</div><p class="live-note">Recorded tokens, including cache. Agents with recorded usage today; not billing cost.</p>':'<p class="empty">No recorded token usage today.</p>';
