...
import { dashboardChartsClient } from './dashboard-charts';
...
export const dashboardClient = codexContextHtml.toString()+';'+dashboardPresentationClient + dashboardRange.toString()+';'+dashboardSince.toString()+';'+ dashboardChartsClient + String.raw`
...
function dashTable(headers,rows){
 const widths=headers[0]==='Agent / Channel'?[170,285,190,140,150,130,175]:headers[0]==='Task / Agent'?[260,120,260,95,95,110,150]:headers[0]==='Agent / Session'?[240,150,150,150,160,180]:[320,180,140];
...
function renderDashboard(data){
  ...
  document.getElementById('overview-attention-panel').hidden=!attention.length;
  document.getElementById('overview-attention').innerHTML=attention.length?dashTable(['Task','Agent','Status'],attention.slice(0,8).map(({a,s,t})=>'<tr'+dashOpenAttrs(a.id,s.sessionId,t.taskId)+' tabindex="0" aria-expanded="false"><td>'+dashText(t.title)+gatewayTaskLabel(t)+'</td><td>'+agentBadge(a.id)+'</td><td>'+dashStatus(t.state)+providerWaitingHtml(t.providerWaiting)+'</td></tr>')):'<p class="muted">No tasks are waiting for a decision or reconciliation.</p>';
  const sessionRows=rows.map(({a,s})=>'<tr class="session-row"'+(s.orchestration?dashOpenAttrs(a.id,s.sessionId):'')+' tabindex="0" aria-expanded="false" data-row-key="'+escHtml(a.id+':'+s.sessionId)+'"><td>'+agentBadge(a.id)+'<br>'+channelBadge(s.source)+'</td><td><span class="session-id">'+dashText(s.sessionId)+'</span><br><small class="ts">Chat '+dashText(s.chatId)+'</small></td><td><span class="cell-clip" title="'+dashText(s.model)+'">'+dashText(s.model||'—')+'</span></td><td>'+dashStatus(sessionDisplayStatus(s))+providerWaitingHtml(s.providerWaiting)+'</td><td>'+ (s.orchestration?dashCount(s.tokenSummary?.agentTokens)+' / '+dashCount(s.tokenSummary?.totalTokens):dashCount(s.tokens))+'<br><small class="ts">'+(s.orchestration?'Agent / total · recorded only':'Legacy process tokens')+'</small></td><td class="dash-tools">'+toolInventory(s.loadedTools,s.usedTools,s.contextTools)+'</td><td>'+ (s.orchestration?'<a class="btn-stream" target="_blank" rel="noopener" href="'+escHtml(dashReportUrl(a.id,s.sessionId))+'">View token report ↗</a>':'')+(s.hasPtyStream&&s.isRunning&&s.mode==='pty-shell'?'<button class="btn-stream" data-agent-id="'+escHtml(a.id)+'" data-session-id="'+escHtml(s.sessionId)+'">Terminal</button>':'')+'</td></tr>');
  ...
  const taskRows=tasks.map(({a,s,t})=>'<tr'+dashOpenAttrs(a.id,s.sessionId,t.taskId)+' tabindex="0" aria-expanded="false" data-row-key="'+escHtml(a.id+':'+t.taskId)+'"><td><strong class="dash-task-title">'+dashText(t.title)+'</strong>'+gatewayTaskLabel(t)+'<small class="ts">'+agentBadge(a.id)+' · '+dashId(t.taskId)+'<br>'+channelBadge(s.source)+'</small></td><td>'+dashStatus(t.state)+providerWaitingHtml(t.providerWaiting)+'</td><td class="session-id">'+dashId(t.gatewayTarget?.sessionId||t.workerSessionId)+'<br><small class="ts">Attempt '+dashId(t.attemptId)+'</small></td><td>'+dashCount(t.tokenSummary?.totalTokens)+'<br><small class="ts">Latest attempt</small></td><td>'+dashCount(t.tokenSummary?.allAttemptsTokens)+'<br><small class="ts">All recorded attempts</small></td><td class="dash-tools">'+(t.gatewayTarget?'—':toolInventory(t.loadedTools,t.usedTools,t.contextTools,t.harness))+'</td><td><span class="cell-clip" title="'+dashText(t.lastTool?.name)+'">'+dashText(t.lastTool?.name||'No recorded tool')+'</span>'+'<br><small class="ts">'+(t.lastTool?.at?dashText(new Date(t.lastTool.at).toLocaleString('en-GB',{timeZone:dashboardTimezone})):'')+'</small></td></tr>');
  document.getElementById('task-results').innerHTML=dashTable(['Task / Agent','Status','Session / Attempt','Latest tokens','Task tokens','Latest attempt tools','Latest activity'],taskRows);
  ...
  document.getElementById('overview-tasks').innerHTML=dashTable(['Task','Agent','Status'],recentWork.slice(0,6).map(({a,s,t})=>'<tr'+dashOpenAttrs(a.id,s.sessionId,t.taskId)+' tabindex="0" aria-expanded="false"><td>'+dashText(t.title)+gatewayTaskLabel(t)+'</td><td>'+agentBadge(a.id)+'</td><td>'+dashStatus(t.state)+providerWaitingHtml(t.providerWaiting)+'</td></tr>'));
  ...
  document.getElementById('tasks-scope').textContent='Worker and Gateway-managed tasks for sessions on the current page. Up to 100 recent tasks per session. Open a row for its execution details and result; token reports contain measured agent and worker turns.';
}
async function dashDetail(agentId,sessionId,taskId,offset=0){
  ...
  try{
    ...
    if(taskId){
      ...
      if(detail.snapshot.computerReport){const r=detail.snapshot.computerReport;body+='<section class="detail-block"><h2>Computer Use result</h2><dl><dt>Outcome</dt><dd>'+dashText(r.status)+' · '+dashText(r.reason)+'</dd><dt>Actions</dt><dd>'+dashCount(r.steps)+'</dd></dl></section>';}
      ...
      body+='<h2>Session</h2><dl><dt>Channel</dt><dd>'+channelBadge(detail.session?.source)+'</dd><dt>Chat</dt><dd>'+dashText(detail.session?.chatId)+'</dd><dt>Created</dt><dd>'+dashText(detail.session?.createdAt?new Date(detail.session.createdAt).toLocaleString('en-GB',{timeZone:dashboardTimezone}):null)+'</dd></dl>';
      ...
      body+=providerWaitingHtml(detail.providerWaiting);
      ...
      body+='<details class="detail-block turn-disclosure session-tasks" data-detail-turn="section:tasks"><summary>Tasks ('+dashCount(detail.totalTasks)+')</summary><div class="dash-mini-list">'+(detail.tasks||[]).map(t=>'<div class="session-task-row"><span class="session-task-title">'+dashText(t.title)+gatewayTaskLabel(t)+'</span>'+dashStatus(t.state)+providerWaitingHtml(t.providerWaiting)+'</div>').join('')+(!detail.tasks?.length?'<p class="muted">No tasks recorded.</p>':'')+'</div></details><h2>Turns & worker attempts</h2>';
      body+=detail.turns.map(t=>'<details class="detail-block turn-disclosure" data-detail-turn="'+dashText(t.id)+'"><summary>'+dashText(t.role)+' · '+dashText(t.category)+' · '+dashText(new Date(t.startedAt).toLocaleString('en-GB',{timeZone:dashboardTimezone}))+' · '+dashCount(t.usage?.totalTokens)+' tokens</summary><p class="live-note">'+dashText(t.id)+' · '+dashText(t.model)+' · '+dashText(t.state)+'</p>'+(t.role==='worker'?'<p>Harness: '+dashText(t.harness==='codex'?'Codex':t.harness==='claude'?'Claude Code':null)+'</p>':'')+(t.taskId?'<p>Task '+dashText(t.taskId)+'</p>':'')+codexContextHtml(t.contextWindow)+(t.gatewayTarget?'—':toolInventory(t.loadedTools,t.usedTools,t.contextTools,t.harness))+(t.contextTools?'<h3>Loaded tools</h3>'+toolNameList(t.contextTools):'')+'<h3>Used tools</h3>'+toolNameList(t.usedTools,t.harness)+(t.inputTexts||[]).map(text=>'<pre>'+dashText(text)+'</pre>').join('')+(t.responseText?'<h3>Response / result</h3><pre>'+dashText(t.responseText)+'</pre>':'')+'</details>').join('');
...
function restoreDashboardDetails(){
 ...
 const html=dashboardCloseButton('dash')+record.html;
