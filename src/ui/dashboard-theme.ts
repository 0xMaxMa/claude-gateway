...
.status-badge{color:var(--status-color);background:color-mix(in srgb,var(--status-color) 12%,var(--panel));border:1px solid color-mix(in srgb,var(--status-color) 28%,var(--line))}
tr[data-dash-agent],.turn-row{cursor:pointer}tr[data-dash-agent]:hover,.turn-row:hover{background:var(--raised)}tr[data-dash-agent]:focus-visible,.turn-row:focus-visible{outline:2px solid var(--accent);outline-offset:-2px}.inline-detail>td{background:var(--raised);padding:22px!important}.inline-detail pre{white-space:pre-wrap;overflow-wrap:anywhere;max-height:350px;overflow:auto}.short-id,.session-id{overflow-wrap:anywhere;white-space:normal;color:var(--text);font-size:12px}.dash-tools{overflow-wrap:anywhere}.detail-block{padding:14px;border:1px solid var(--line);border-radius:10px;margin:12px 0}.detail-block h3{margin:0 0 8px}
`;
