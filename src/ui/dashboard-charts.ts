...
const chartColors=['var(--palette-sky)','var(--palette-lavender)','var(--palette-mint)','var(--palette-pink)','var(--palette-peach)','var(--palette-yellow)'];
...
function chartAgents(data,element){
 ...
 return '<div class="agent-column-plot"><div class="chart-y-labels"><span>'+compactNumber(peak)+'</span><span>'+compactNumber(peak/2)+'</span><span>0</span></div><div class="agent-columns">'+top.map(a=>'<div class="agent-column" style="--agent-hue:'+agentHue(a.id)+'" tabindex="0" role="img" aria-label="'+escHtml(a.id+': Agent '+compactNumber(a.agent)+', workers '+compactNumber(a.worker)+', total '+compactNumber(a.total))+'"><div class="agent-bar-space"><div class="agent-bar" style="height:'+Math.max(1,a.total/peak*100)+'%"><span class="agent-bar-total">'+compactNumber(a.total)+'</span></div></div><span class="agent-bar-label">'+agentBadge(a.id)+'</span><span class="chart-popover"><strong>'+escHtml(a.id)+'</strong><br>Agent · '+compactNumber(a.agent)+'<br>Workers · '+compactNumber(a.worker)+'<br>Total · '+compactNumber(a.total)+'</span></div>').join('')+'</div></div><div class="chart-key"><span>Agent + worker tokens · colors match Agents below</span></div><p class="live-note">Top '+top.length+' of '+agents.length+' active agents · ranked by recorded tokens.</p>';
}
...
function chartReuse(data){
 ...
 const pieces=[['Reused input',sum.read,'var(--token-agent)','Cache read · existing input used again'],['New · saved for reuse',sum.write,'var(--token-worker)','Cache write · new input stored for later requests'],['New · not cached',sum.fresh,'var(--token-report)','Fresh input · newly processed input']];
