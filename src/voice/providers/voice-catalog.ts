import { openRouterConnection, openRouterVoiceModels } from './openrouter';
...
export interface VoiceChoice { id: string; name: string; gender?: string; supported_model_ids?: string[] }
...
const catalogs: Record<string, Catalog> = {
  'managed:elevenlabs': {
    key: () => { const { base, key } = upstreamVoiceConnection('elevenlabs', true); return base.toString() + '\0' + key; },
    list: async () => {
...
for (const provider of ['paxalabs', 'upstream:paxalabs', 'managed:paxalabs']) catalogs[provider] = {
...
for (const provider of ['openrouter','upstream:openrouter']) catalogs[provider] = {
  key: () => {const {base,key}=openRouterConnection(provider);return key ? base.toString()+'\0'+key : '';},
  list: async () => {
    const models=await openRouterVoiceModels(provider);
    const voices=new Map<string,VoiceChoice>();
    for (const model of models) for (const id of model.supported_voices ?? []) {
      if (!voices.has(id)) voices.set(id,{id,name:id,supported_model_ids:[]});
      voices.get(id)!.supported_model_ids!.push(model.id);
    }
    return [...voices.values()];
  },
};
const cache=new Map<string,{until:number;voices:VoiceChoice[]}>();
export async function voiceChoices(config:{provider:string;voiceId:string;model?:string}):Promise<VoiceChoice[]> {
  config = {...config, provider: canonicalVoiceProvider(config.provider)};
  ...
  const compatible = (voices:VoiceChoice[]) => config.model ? voices.filter(v => !v.supported_model_ids || v.supported_model_ids.includes(nativeVoiceModel(config.provider,config.model!))) : voices;
  const cached=cache.get(key);if(cached&&cached.until>Date.now())return compatible(cached.voices);
  const voices=await catalog.list(credential);
  if(cache.size>=20)cache.delete(cache.keys().next().value!);
  cache.set(key,{until:Date.now()+300000,voices});return compatible(voices);
}

// Keep Auto stable for this gateway process, separately for each provider/account.
const automatic = new Map<string, VoiceChoice>();
export async function resolveVoiceId(config: {provider: string; voiceId: string; model?: string}): Promise<string> {
  config = {...config, provider: canonicalVoiceProvider(config.provider)};
  ...
  const key = createHash('sha256').update(config.provider + '\0' + credential + '\0' + (config.model ?? '')).digest('hex');
  const selected = automatic.get(key);
  if (selected) return selected.id;
  const allVoices = await voiceChoices(config);
  const model = config.model ? nativeVoiceModel(config.provider,config.model) : undefined;
  const voices = allVoices.filter(v => !v.supported_model_ids || (model && v.supported_model_ids.includes(model)));
