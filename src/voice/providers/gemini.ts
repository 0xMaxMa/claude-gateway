...
export async function geminiGenerate(provider: string, model: string, body: unknown, signal: AbortSignal, request: typeof fetch = fetch, operation: 'STT' | 'TTS' | 'VOICE' = 'VOICE') {
  ...
  if (!response.ok) throw await providerHttpError(operation, response);
  const result = await response.json() as { candidates?: Array<{ finishReason?: string; content?: { parts?: Array<{ text?: string; audioTranscription?: { text?: string }; inlineData?: { data: string; mimeType: string } }> } }> };
  voiceTiming('provider_body_received');
  return result;
}
...
export class GeminiTts implements TtsProvider {
  ...
  async *synthesize(options: Parameters<TtsProvider['synthesize']>[0]) {
    ...
    if (!text.trim()) return;
    const result = await geminiGenerate(this.id, this.model, { contents: [{ parts: [{ text }] }], generationConfig: { responseModalities: ['AUDIO'], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: options.voiceId } } } } }, options.signal, this.request, 'TTS');
    const candidate = result.candidates?.[0];
