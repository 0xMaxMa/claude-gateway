...
export function upstreamVoiceConnection(provider: 'elevenlabs' | 'paxalabs' | 'gemini' | 'openrouter' = 'elevenlabs', managed = false): { base: URL; key: string } {
  ...
  if (
    ...
    throw new VoiceError('INVALID_UPSTREAM_VOICE_URL');
  base.pathname = `/v1/voice/${managed ? 'managed/' : ''}${provider}/`;
...
export const upstreamVoiceSocket: SocketFactory = relayVoiceSocket(false);
export const managedVoiceSocket: SocketFactory = relayVoiceSocket(true);
function relayVoiceSocket(managed: boolean): SocketFactory { return async (
  upstream,
  _headers,
  signal,
) => {
  const { base, key } = upstreamVoiceConnection('elevenlabs', managed);
