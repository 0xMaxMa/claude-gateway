...
export function providerVoiceError(prefix: 'STT' | 'TTS' | 'VOICE', status?: number, payload?: unknown): VoiceError {
  ...
  if (candidates.includes('MANAGED_VOICE_QUOTA_EXHAUSTED')) return new VoiceError('MANAGED_VOICE_QUOTA_EXHAUSTED');
...
export function describeVoiceError(error: unknown) {
  ...
  if (category === 'unknown') {
    ...
    else if (/STT_CONNECTION_CLOSED|PROVIDER_CONNECTION|PROVIDER_SEND|fetch failed|ECONN|ENOTFOUND/.test(code)) category = 'network';
...
export async function voiceProviderRequest(url: URL | string, init: RequestInit, request: typeof fetch = fetch): Promise<Response> {
  voiceTiming('provider_request_started');
  try { const response = await request(url, init); voiceTiming('provider_headers_received', { httpStatus: response.status }); return response; }
  catch (error) {
