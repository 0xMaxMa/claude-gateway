...
async function downloadSlackImage(
  ...
  const dest = path.join(os.tmpdir(), `slack-img-${suffix.replace(/[^A-Za-z0-9-]/g,'')}.${audioMime?({'audio/mpeg':'mp3','audio/mp4':'m4a','audio/ogg':'ogg','audio/wav':'wav','audio/webm':'webm'} as Record<string,string>)[audioMime]??'audio':sniffImageExt(buf)}`);
...
export function createSlackWebhookHandler(
  ...
  const handlePost = async (req: Request, res: Response): Promise<void> => {
    ...
    const imageFile = event.files?.find(
      (f) => (f.mimetype?.startsWith('image/') || !!(orchestrationConfig?.enabled&&(orchestrationConfig.channels??['api']).includes('slack')&&isSlackAudio(f))) && (f.url_private_download||f.url_private),
    );
    const mediaUrl=imageFile?.url_private_download??imageFile?.url_private;
    if (token && imageFile && mediaUrl) {
      try {
        const imgPath = await downloadSlackImage(token, mediaUrl, imageFile.id,slackAudioMime(imageFile));
        if (imgPath) { norm.meta.image_path = imgPath;norm.meta.media_ephemeral='1';if(isSlackAudio(imageFile)){norm.meta.media_type='audio';norm.meta.attachment_kind='voice';if(!norm.content)norm.content='(voice message)';} }
      } catch (err) {
        logger.warn('Slack webhook: attachment download failed', {
          fileId: imageFile.id,
