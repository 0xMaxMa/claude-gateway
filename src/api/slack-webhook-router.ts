...
export function createSlackWebhookHandler(
  ...
  const handlePost = async (req: Request, res: Response): Promise<void> => {
    ...
    if(payload.type==='block_actions'||payload.type==='slash_command'){
      ...
      if(slash?!['/restart','/clear','/compact','/session','/sessions','/voice','/voices','/tasks','/task_question','/stop','/help'].includes(command):!/^orch:(?:[a-f0-9-]{36}|q:[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}:(?:snooze|mute))$/.test(data)){res.status(200).json({text:'Unknown command.'});return;}
