...
export function createApiRouter(
  ...
  // Session-scoped Task controls share the same principal checks as chat.
  for (const route of ['/v1/agents/:agentId/sessions/:sessionId/tasks', '/v1/agents/:agentId/sessions/:sessionId/tasks/:taskId']) {
    router.get(route, auth, async (req: Request, res: Response) => {
      ...
      if (!isValidSessionId(sessionId) || (taskId !== undefined && !isValidSessionId(taskId))) {
        res.status(400).json({ error: 'Invalid session or task ID' }); return;
      }
      ...
      if ((req.query.page !== undefined && (typeof req.query.page !== 'string' || !/^\d+$/.test(req.query.page))) ||
          (req.query.page_size !== undefined && (typeof req.query.page_size !== 'string' || !/^\d+$/.test(req.query.page_size))) ||
          !Number.isSafeInteger(page) || page < 0 || !Number.isSafeInteger(pageSize) || pageSize < 1 || pageSize > 100 ||
          (req.query.all !== undefined && req.query.all !== 'true' && req.query.all !== 'false')) {
        res.status(400).json({ error: 'Invalid task pagination or all filter' }); return;
      }
      try {
        const principal = apiPrincipal(key);
        res.json(taskId ? { task: await runner.showApiTask(sessionId, principal, taskId) } :
          await runner.listApiTasks(sessionId, principal, page, pageSize, req.query.all === 'true'));
      } catch (error) {
        if ((error as Error).message === 'ORCHESTRATION_DISABLED') { res.status(409).json({ error: 'ORCHESTRATION_DISABLED' }); return; }
        res.status(403).json({ error: 'Task unavailable for this principal' });
      }
    });
  }
