...
function printHelp(requested: boolean): void {
  const ch = PAIRING_CHANNELS.join('|');
  // The channel list is interpolated, so the gap cannot be hand-counted — the
  // rows drifted out of alignment as soon as a channel was added. Pad to the
  // widest usage instead.
  const rows: Array<[string, string]> = [
    [`channels pending --agent <id> [--channel ${ch}]`, 'List pending requests (both channels if omitted)'],
    [`channels approve --agent <id> --channel <${ch}> --code <code>`, 'Approve a pending request'],
    [`channels deny    --agent <id> --channel <${ch}> --code <code>`, 'Deny and remove a pending request'],
  ];
  ...
  writeCommandHelp(
    requested,
    'channels',
    'manage incoming Telegram/Discord pairing requests',
    'claude-gateway channels <pending|approve|deny> --agent <id> [--flags]',
    lines,
  );
}
