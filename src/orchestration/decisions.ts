...
export class DecisionService {
  ...
  acknowledge(receipt: DecisionReceipt, text: string, spoken?: string, allowCompleted = false): string {
    if (!text.trim() || Buffer.byteLength(text) > 2400) throw new OrchestrationError('INVALID_INPUT');
    return this.store.transaction(() => {
      ...
      if (!current) throw new OrchestrationError('STALE_DECISION');
      ...
      this.store.run('INSERT INTO conversation_decisions VALUES(?,?,?,?,?,?,?,?,?,?,?)', ackDecisionId, current.conversation_id, current.epoch,
        'acknowledgement', null, 'completed', current.session_id, current.input_ids_json, '[]', now, now);
      ...
      for (const inputId of receipt.inputIds) this.historyOperation(`input:${inputId}`, String(current.conversation_id), inputId, null);
      this.historyOperation(`response:${id}`, String(current.conversation_id), null, id);
      if (spoken) this.store.run('INSERT INTO response_speech VALUES(?,?)', id, spoken);
      ...
      if (binding && binding.channel !== 'api') this.deliver?.(id, String(input!.binding_id), text);
  ...
  finish(receipt: DecisionReceipt, text: string, state: 'completed' | 'interrupted' | 'failed' = 'completed', spoken?: string, deliver = true): string {
    ...
    return this.store.transaction(() => {
      ...
      if (state === 'completed' && text.trim()) this.store.run(`UPDATE task_files SET response_id=? WHERE response_id IS NULL AND task_id IN
