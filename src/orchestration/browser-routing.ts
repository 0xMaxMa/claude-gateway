...
export function browserRouting(agent: AgentConfig, gateway: GatewayConfig, hostConnectors = true): string {
  ...
  const jev = jevAllowed(gateway,agent) && gateway.gateway.jev?.features?.browserTasks?.enabled === true && Boolean(gateway.gateway.jev?.browser);
