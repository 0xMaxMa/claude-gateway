import { resolveStoredReply } from './reply-context';
...
export class OrchestrationHistoryWriter {
  ...
  async write(operationId: string): Promise<void> {
    ...
    if (input?.store_user_message === 0 || (response && !String(response.generated_text).trim())) {
    ...
    const message = { operationId, role: input ? 'user' as const : 'assistant' as const,
      content: String(input?.text ?? response?.generated_text), ts: Number(input?.created_at ?? response?.completed_at ?? response?.created_at) };
    ...
    if(input)ingress.metadata=resolveStoredReply(this.orchestration,{agentId:this.orchestration.agentId,agentSessionId:sessionId,source,accountId:String(conversation.account_id),chatId,threadKey:String(conversation.thread_key),principalId:String(input.principal_id)},ingress.metadata);
    ...
    const ref = this.history.insertMessageOnce(operationId, { ...message, ...ingress.metadata, repliedToMessageId: ingress.metadata?.repliedMessageId, repliedToText: ingress.metadata?.repliedText, repliedToUser: ingress.metadata?.repliedSender, mediaFiles: mediaFiles.length ? mediaFiles : undefined, chatId: `${source}-${chatId}`, sessionId, source });
