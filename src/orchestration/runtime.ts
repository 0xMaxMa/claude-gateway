...
export class AgentOrchestrationRuntime {
  ...
  private async run(input: AcceptInput, capabilities: ExecutionCapabilities, options: { timeoutMs: number; model?: string; onText?: (text: string) => void; onTool?: (event: ToolActivity) => void }): Promise<string> {
    ...
    try {
      ...
      if(this.bridge.computerEnabled?.())ticket.profile.overlay+='\nComputer Use: control desktop applications only through capabilities_list scope=computer and task_spawn target_profile=gateway-managed with gateway_target adapter=computer and its returned session_id. Pairing and local app consent are required. Do not substitute SSH, shell commands or a direct worker. Follow-up work uses task_update when_ready with the complete goal. Browser-only requests still use Browser Use. Do not claim completion without independent verification.';
