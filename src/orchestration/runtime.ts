...
export class AgentOrchestrationRuntime {
  ...
  static async open(agent: AgentConfig, gateway: GatewayConfig, root: string, sessions: SessionStore, historyDb: HistoryDB,
    ...
    gatewayAdapters.set('computer',new ComputerTaskAdapter({agentId:agent.id,root:join(root,'computer-requests'),connectors:new ComputerConnectors(gateway,agent),allowed:computerAllowed,
      ...
      experience:task=>new ExperienceLibrary({...gateway.gateway.jev?.experience,directory:join(root,'experience'),scope:JSON.stringify([agent.id,task.ownerPrincipalId,task.conversationId])}),
    ...
    for (const conversation of this.questionControls.initialReviews([...this.active.keys()])) {
      this.store.compose(() => {
        this.store.acceptInput({ scope: { agentId: this.agent.id, agentSessionId: String(conversation.agent_session_id), source: conversation.source as ConversationScope['source'],
          ...
          text: 'Review the new pending task questions. Use task_question action=ask to ask for the missing decision naturally in one separate message. For a browser missing-field question, first inspect task_status and use task_answer if established user facts already provide the value. Do not ask for that value again. For genuinely missing facts or new consent, ask the user. Do not start unrelated work or repeat the question in a final reply. If no question remains, stay silent.',
