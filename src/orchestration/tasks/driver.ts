...
export class ClaudeWorkerDriver implements WorkerDriver {
  ...
  async start(task: TaskSnapshot, attempt: TaskAttempt, capacityReserved = false): Promise<WorkerHandle> {
    ...
    try {
      ...
      const reply = resolveStoredReply(this.tasks.store,{agentId:task.agentId,agentSessionId:task.agentSessionId,source:conversation.source as import('../types').ConversationScope['source'],accountId:String(conversation.account_id),chatId:String(conversation.chat_id),threadKey:String(conversation.thread_key),principalId:String(input.principal_id)},JSON.parse(String(input.ingress_json??'{}')).metadata);
      const attachments = [...new Set([...(JSON.parse(String(input.attachment_refs_json)) as string[]),...(reply?.repliedAttachmentIds??[])])].map(ref => MediaStore.resolvePath(join(this.agent.workspace, '../..'), this.agent.id, ref));
      const references = revision.contextRefs.map(ref => {
        ...
        if (inputRef) return { ref, kind: 'input', text: inputRef.text, replyContext: storedReplyContext(inputRef.ingress_json), attachments: (JSON.parse(String(inputRef.attachment_refs_json)) as string[]).map(path => MediaStore.resolvePath(join(this.agent.workspace, '../..'), this.agent.id, path)) };
