...
export class TaskControls {
  ...
  private view(task: TaskSnapshot) {
    ...
    return { revision:task.revision, appliedRevision:task.appliedRevision, pendingRevision:task.gatewayTarget && task.activeAttemptId && task.revision>1 && task.revision>task.appliedRevision ? task.revision : undefined, browserReport: task.browserReport, gatewayTarget: task.gatewayTarget, executionType: task.gatewayTarget ? 'gateway-managed' as const : 'worker' as const, harness, timing: timingTotals(task), timingMeasuredFrom: task.timing?.measuredFrom, supervision: task.supervision, progressText, activityDetails, startedAt, finishedAt, taskId: task.taskId, title: task.title.slice(0,200), state: task.state, replacedByTaskId: task.replacedByTaskId,
  ...
  list(sessionId: string, principalId: string, page = 0, pageSize = 10, includeFinished = false) {
    if (!Number.isSafeInteger(page) || page < 0 || !Number.isSafeInteger(pageSize) || pageSize < 1 || pageSize > 100) throw new OrchestrationError('INVALID_TASK_PAGE');
