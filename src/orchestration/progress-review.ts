...
// is published, so a partial draft or a control JSON object cannot leak. The field
// contract is carried by the invariant ORCHESTRATION_RESPONSE_SCHEMA declared on every
// turn; this overlay is the per-turn text that asks for notify_user on a review turn.
...
/** A review turn's decision, and WHY it is silent.
 * 'reported'  — a new update to publish.
 * 'silent'    — the model explicitly chose not to notify; nothing was lost.
 * 'duplicate' — a valid update that repeats one already communicated.
 * 'unparsed'  — no valid decision could be read at all. The update (if any) is dropped,
 *               so the caller MUST surface it rather than treat it as a quiet review. */
export type ProgressReviewOutcome = 'reported' | 'silent' | 'duplicate' | 'unparsed';
export function progressReviewResult(raw: string, previous: string[]): { display: string; spoken: string; silent: boolean; outcome: ProgressReviewOutcome } {
  ...
  for (const candidate of [normalized, ...starts.map(index => normalized.slice(index))]) {
    try {
      ...
      if (value.notify_user === false) return {display:'',spoken:'',silent:true,outcome:'silent'};
      ...
      if (previous.some(text => canonical(text) === canonical(display))) return {display:'',spoken:'',silent:true,outcome:'duplicate'};
      return {display,spoken:typeof value.spoken_text === 'string' && value.spoken_text.length <= SPOKEN_TEXT_LIMIT ? value.spoken_text.trim() : '',silent:false,outcome:'reported'};
    } catch { /* Only an explicit valid reporting decision can publish. */ }
  }
  // Still fail closed — unvalidated review prose must never reach the user or TTS — but
  // report it as 'unparsed' so the caller can log the dropped update. Silently treating a
  // malformed review as a deliberate silence is exactly the no-silent-failures violation.
  return {display:'',spoken:'',silent:true,outcome:'unparsed'};
}
