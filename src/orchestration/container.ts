...
export function hasNoNewPrivileges(options: unknown): boolean {
  if (!Array.isArray(options)) return false;
  let enabled = false;
  for (const option of options) {
    if (typeof option !== 'string') return false;
    if (!option.startsWith('no-new-privileges')) continue;
    if (option === 'no-new-privileges') { enabled = true; continue; }
    const match = /^no-new-privileges[=:](true|True|TRUE|1|false|False|FALSE|0)$/.exec(option);
    if (!match) return false;
    enabled = ['true','True','TRUE','1'].includes(match[1]);
  }
  return enabled;
}

export async function validateContainerInspection(agent: AgentConfig, c: any, options: { requireRunning?: boolean } = {}): Promise<void> {
  const h = c.HostConfig;
  if ((options.requireRunning !== false && !c.State?.Running) || !h || h.Privileged || (h.NetworkMode === 'host' || String(h.NetworkMode).startsWith('container:')) || h.PidMode || !['private','none',''].includes(h.IpcMode ?? '') || h.CapAdd?.length || !(h.CapDrop ?? []).includes('ALL') || !hasNoNewPrivileges(h.SecurityOpt) || h.Devices?.length) throw new OrchestrationError('CONTAINER_ISOLATION_REQUIRED');
