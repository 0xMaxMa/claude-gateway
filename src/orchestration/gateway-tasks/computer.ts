...
interface Receipt {revision:number;taskId:string;requestId:string;principal:string;conversation:string;ended:boolean;trace?:ComputerProgress[];verification?:{id:string;at:number;revision:number;connectionHash:string};snapshot?:{observedAt:number;state:any;screenshotError?:string;screenshot?:{type:'image';mimeType:'image/jpeg';data:string;generation:string;capturedAt:number}};operationId?:string;outcome?:WorkerOutcome}
...
export class ComputerTaskAdapter implements GatewayTaskAdapter {
 ...
 async submit(t:TaskSnapshot,r:string,goal:string,answers:TaskRevision['answers']=[],_requestConsent=false,preparedInputs:TaskRevision['computerInputs']=[]){
  ...
  const done=withComputerConnection(connection,async client=>{
   ...
   let leaseToken:string|undefined;let fieldRequest:{label:string;application?:string;windowTitle?:string;role?:string;reason:'missing'}|undefined;
   ...
   const result=await runComputerUse({revision:t.revision,preparedInputs,goal:goal+recoveryNote+(answers?.length?'\nKnown answers: '+JSON.stringify(answers.map(a=>a.text)):'')},{authorized,interruptSignal:interrupt.signal,
    call:async(name,args,s)=>{if(name!=='computer_release'&&!authorized())throw Error('ACCESS_DENIED');const response=await client.callTool({name,arguments:{...args,...b.scope}},undefined,{signal:s,timeout:20000});const text=(response.content as any[])?.find(x=>x.type==='text')?.text;if(typeof text!=='string'||text.length>262144)throw Error('COMPUTER_RESPONSE_INVALID');const body=JSON.parse(text);if(response.isError){const code=body?.error;const known=['ACCESSIBILITY_PERMISSION_REQUIRED','SCREEN_RECORDING_PERMISSION_REQUIRED','APPLICATION_NOT_ALLOWED','COMPUTER_BUSY','COMPUTER_RECONCILIATION_REQUIRED','CONSENT_REQUIRED','ACCESS_REVOKED','DEVICE_OFFLINE','NATIVE_FAILURE','NATIVE_PROCESS_EXITED','INVALID_NATIVE_RESPONSE','SCREENSHOT_UNSUPPORTED','SCREENSHOT_SENSITIVE_CONTENT','SCREENSHOT_CAPTURE_FAILED','SCREENSHOT_WINDOW_UNAVAILABLE','SCREENSHOT_TOO_LARGE','STALE_OBSERVATION','NATIVE_BUSY','OBSERVATION_FAILED','COMPUTER_CLOSED','COMPUTER_NOT_OWNED'];throw Error(known.includes(code)?code:'COMPUTER_TOOL_FAILED');}if(name==='computer_acquire')leaseToken=body.lease_token;return body;},
    observation:state=>{const screenshot=receipt.snapshot?.state.generation===state.generation?receipt.snapshot?.screenshot:undefined;receipt.snapshot={observedAt:Date.now(),state,...(screenshot?{screenshot}:{})};this.write(t,r,receipt);},
    snapshot:async(state:any,s:AbortSignal)=>{
     ...
     try{
      ...
      receipt.snapshot={observedAt:Date.now(),state,...(image?{screenshot:image}:{screenshotError:'COMPUTER_SCREENSHOT_UNAVAILABLE'})};
 ...
 async computerEvidence(t:TaskSnapshot,mode:'recorded'|'fresh'|'screenshot',signal=AbortSignal.timeout(15000)){
  ...
  if(!r||receipt?.operationId)throw Error('COMPUTER_RECONCILIATION_REQUIRED');
