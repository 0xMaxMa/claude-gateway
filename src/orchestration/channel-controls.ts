...
export class ChannelControls {
  ...
  async handle(s:ControlScope,text:string):Promise<ControlMenu> {
    ...
    if(action.kind==='detail'||action.kind==='cancel'){
      ...
      return {text:`${task.title}\nStatus: ${task.state}${task.pendingRevision ? `\nUpdated instructions accepted (revision ${task.pendingRevision}); waiting for the current request to settle.` : ''}\nType: ${task.executionType === 'gateway-managed' ? 'Gateway-managed task' : 'Worker task'}${task.gatewayTarget ? `\nTarget: ${task.gatewayTarget.adapter === 'browser' ? 'Browser' : task.gatewayTarget.adapter === 'computer' ? 'Computer Use' : 'Safemode'} · ${task.gatewayTarget.name}\nSession: ${task.gatewayTarget.sessionId}` : ''}${task.computerReport ? `\nComputer Use: ${task.computerReport.status} · ${task.computerReport.reason}\nActions: ${task.computerReport.steps} · Evaluations: ${task.computerReport.evaluations??'—'}\nPhase: ${task.computerReport.phase??task.computerReport.status}` : ''}${task.browserReport ? `\nBrowser: ${task.browserReport.status} · ${task.browserReport.reason}\nActions: ${task.browserReport.steps} · Evaluations: ${task.browserReport.evaluations}${task.browserReport.lastAction ? `\nLast action: ${task.browserReport.lastAction.operation} · ${task.browserReport.lastAction.outcome}` : ''}` : ''}\nElapsed: ${formatTaskElapsed(task.startedAt,task.finishedAt)}${task.harness ? `\nHarness: ${task.harness === 'codex' ? 'Codex' : 'Claude Code'}` : ''}${formatTiming(task.timing)?`\n\nTime breakdown\n${formatTiming(task.timing)}`:''}${task.progress?`\n\nLatest progress:\n${task.progress}`:''}${task.question?`\n\nWaiting for your input:\n${task.question}`:''}`,buttons:[...(task.canStop?[b(task.state==='needs_reconciliation'?'🔄 Retry cleanup':'🔴 Stop task',{kind:'cancel',id:task.taskId})]:[]),b('🔄 Refresh',{kind:'detail',id:task.taskId}),b('Back',{kind:'tasks'}),dismiss()]};
    }
