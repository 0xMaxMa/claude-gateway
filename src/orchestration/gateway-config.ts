...
export function migrateAgentVoiceConfig(document: { gateway?: any; agents?: any[] }): boolean {
  ...
  for (const agent of document.agents ?? []) {
    ...
    for (const role of ['tts','stt','notes']) {
      const item = agent.voice?.[role];
      if (typeof item?.provider === 'string' && canonicalVoiceProvider(item.provider) !== item.provider) { item.provider = canonicalVoiceProvider(item.provider); changed = true; }
    }
    ...
    if (legacyGlobal?.voice || legacy?.voice) {
      ...
      for (const role of ['tts','stt','notes']) if (typeof voice[role]?.provider === 'string') voice[role].provider = canonicalVoiceProvider(voice[role].provider);
      agent.voice = voice;
    }
