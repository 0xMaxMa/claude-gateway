...
export function inferenceFailureMessage(error: unknown): string | undefined {
  ...
  const auth = /\b(?:authentication failed|invalid api key|unauthorized|invalid credentials|HTTP\s*401|API Error:\s*401)\b/i.test(detail);
  ...
  const reset = detail.match(/\b(?:resets?|resetting|retry(?:ing)?(?: after| in)?|try again in)\s+(?:in\s+|after\s+)?(\d{1,4}\s*(?:seconds?|minutes?|hours?|days?)|\d{1,2}:\d{2}(?::\d{2})?\s*(?:UTC|GMT)?)/i);
  const guidance = reset ? ` Try again ${/\breset/i.test(reset[0]) ? 'after the limit resets in' : 'in'} ${reset[1].trim()}.`
