...
export class ConversationIntake {
  ...
  choose(context: CommandContext, choice: IntakeChoice) {
    ...
    if (choice.mode === 'resolve' && choice.task_id && !this.store.get(
      "SELECT action_id FROM task_commands WHERE conversation_id=? AND principal_id=? AND decision_id=? AND task_id=? AND command_type IN ('spawn','update','answer') LIMIT 1",
      context.conversationId, context.principalId, context.decisionId, choice.task_id)) throw new OrchestrationError('INTAKE_RESOLUTION_UNCOMMITTED');
