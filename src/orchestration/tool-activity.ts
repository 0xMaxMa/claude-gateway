  ...
  return line => {
    ...
    for (const block of event.message.content) {
      if (block.type === 'tool_use' && typeof block.id === 'string' && typeof block.name === 'string' && !calls.has(block.id) && calls.size < 2000) {
        const actual = executionTool(block);
        calls.set(block.id, actual.name);
        const input: Record<string, unknown> = {};
        for (const [key, value] of Object.entries(actual.input).slice(0, 16)) {
        ...
        publish({ type: 'tool_use', id: block.id, name: actual.name, input });
      }
